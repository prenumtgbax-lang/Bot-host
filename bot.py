...
def _auto_install_missing() -> None:
    ...
    # Try several install strategies â€” different hosts have different
    ...
    for cmd in strategies:
        try:
            ...
            print("[setup] install ok â€” continuing boot")
            return
...
# â”€â”€ TELEGRAM BOT API 9.4 â€” BUTTON STYLE SUPPORT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
_SEC_PATTERNS = {
    # â”€â”€ Real data theft â€” actively reading & exfiltrating server files â”€â”€
    "ðŸ”´ Data Theft": [
        # Must have a specific system directory name after the slash (not '/' alone)
        (r'os\.walk\s*\(\s*["\'][/\\](?:root|home|etc|var|proc)["\']',
                                                  "Root/system directory walk â€” server files chura raha hai"),
        ...
        (r'glob\.glob\s*\(["\'][/\\]\*',          "Root glob scan â€” server files dhundh raha hai"),
        (r'shutil\.copy.*["\'][/\\]root',         "/root se copy kar raha hai"),
        (r'ROOT_DIR\s*=\s*["\'][/\\]["\']',       "Root directory target kar raha hai"),
    ],
    # â”€â”€ True backdoors â€” code that executes arbitrary commands â”€â”€
    ...
    "ðŸ”´ Backdoor": [
        ...
        (r'marshal\.loads\s*\(',                  "Marshalled bytecode â€” obfuscated execution"),
    ],
    # â”€â”€ Exposed credentials â€” actual tokens/secrets in plain text â”€â”€
    "ðŸ”´ Exposed Credentials": [
        # BOT_TOKEN_REGEX handled separately in _sec_static_scan
    ],
    # â”€â”€ Obfuscation â€” actively hiding intent â”€â”€
    "ðŸŸ¡ Obfuscation": [
        (r'base64\.b64decode\s*\(.*\)\s*[\)\s]*\bexec\b',
                                                  "Base64 decode + execute â€” hidden code"),
        (r'(?:\\x[0-9a-fA-F]{2}){6,}',           "Long hex string â€” obfuscated code"),
        (r'zlib\.decompress\s*\(.*\)\s*[\)\s]*\bexec\b',
                                                  "Compressed + executed hidden code"),
    ],
    # â”€â”€ Suspicious network â€” sending data out to KNOWN malicious endpoints â”€â”€
    "ðŸŸ¡ Suspicious Network": [
        ...
        (r'open\s*\(\s*["\'][/\\](?:root|etc|proc|sys).*(?:requests|urllib).*(?:post|put)',
                                                  "System file HTTP POST â€” data exfiltration"),
        (r'pastebin\.com/raw',                    "Pastebin raw fetch â€” remote code load"),
    ],
    # â”€â”€ Resource abuse â”€â”€
    "ðŸŸ  Resource Abuse": [
        (r'multiprocessing\.Pool\s*\(\s*(?:None|\d{3,})',
                                                  "Massive process pool â€” resource abuse"),
...
def _sec_static_scan(code: str) -> dict:
    ...
    for category, pattern_list in _SEC_PATTERNS.items():
        ...
        for pattern, description in pattern_list:
            # No DOTALL â€” keeps .* within a single line so multi-token patterns
    ...
    if tokens:
        results.setdefault("ðŸ”´ Exposed Credentials", [])
        results["ðŸ”´ Exposed Credentials"].append(f"Bot Token mila: {tokens[0][:15]}...")
    return results
            ...
            if isinstance(func, _ast.Name) and func.id in ('eval', 'exec'):
                if node.args:
                    ...
                    if isinstance(arg0, _ast.Call):
                        findings.append(f"Dangerous: {func.id}() â€” dynamic code execution")
                    elif isinstance(arg0, _ast.Attribute):
                        findings.append(f"Dangerous: {func.id}() â€” attribute-based input")
            # __import__('os') â€” dynamic OS import (AST-only to skip string literals)
            if isinstance(func, _ast.Name) and func.id == '__import__':
                if node.args and isinstance(node.args[0], _ast.Constant):
                    if node.args[0].value == 'os':
                        findings.append("Dynamic __import__('os') â€” code injection")
    return findings
...
def _sec_calculate_risk(static_findings: dict, ast_findings: List[str]) -> int:
    ...
    weights = {
        "ðŸ”´ Data Theft":          40,
        "ðŸ”´ Backdoor":            40,
        # Having a token in code is bad practice but NOT necessarily theft â€”
        # many bots hardcode their token. Weight kept low so it alone can't
        # reach the DANGEROUS threshold.
        "ðŸ”´ Exposed Credentials": 10,
        "ðŸŸ¡ Suspicious Network":  12,
        "ðŸŸ¡ Obfuscation":         10,
        "ðŸŸ  Resource Abuse":       8,
    }
...
def _sec_get_verdict(risk_score: int, static_findings: dict) -> Tuple[str, str]:
    ...
    # Exposed Credentials alone â†’ SUSPICIOUS (warn user, don't block).
    has_blocking = any(
        static_findings.get(c)
        for c in ("ðŸ”´ Data Theft", "ðŸ”´ Backdoor")
    )
    has_credentials = bool(static_findings.get("ðŸ”´ Exposed Credentials"))
    ...
    if risk_score >= 85:
        return "DANGEROUS", "REJECT"
    # Hardcoded token alone â†’ warn but allow (MANUAL_REVIEW)
...
def _sec_scan_code(code: str, filename: str = "file.py") -> dict:
    ...
    if verdict == "DANGEROUS":
        summary = f"âš ï¸ File DANGEROUS hai! {len(all_threats)} threats mili hain."
    elif verdict == "SUSPICIOUS":
        summary = "ðŸ” File suspicious hai. Admin se manual review karwao."
    else:
        summary = "âœ… File safe lagti hai. Koi major threat nahi mila."
    ...
    try:
                ...
                for name in z.namelist():
                    if name.startswith('/') or '..' in name:
                        return {"verdict": "DANGEROUS", "risk_score": 99,
                                "findings": {"ðŸ”´ Zip Slip Attack": ["Dangerous file paths in ZIP!"]},
        ...
        if not py_files:
            return {"verdict": "SUSPICIOUS", "risk_score": 20,
                    "findings": {"ðŸŸ¡ Warning": ["Koi .py file nahi mili archive mein"]},
...
def _scan_file(file_path: str) -> dict:
    """Main entry â€” scan any uploaded file before saving."""
    ...
    try:
        if filename.lower().endswith(('.zip', '.tar.gz', '.tgz', '.tar')):
            ...
            return {"verdict": "SUSPICIOUS", "risk_score": 30,
                    "findings": {"ðŸŸ¡ Warning": [f"Unknown file type: {filename}"]},
                    "ast_findings": [], "recommendation": "MANUAL_REVIEW",
...
# â”€â”€ External scanner module (security_scanner_free.py) â”€â”€
...
# are used by _combined_scan â†’ _run_security_scan â†’ _handle_bot_upload.
try:
    ...
    import sys as _sys
    print(f"[security] security_scanner_free.py not found â€” using built-in scanner ({_ssf_err})", file=_sys.stderr)
    # Fall back to built-in _scan_file defined above


# â”€â”€ AI-powered scanner (OpenRouter free model â€” no API key needed) â”€â”€
...
1. Data theft â€” reading/sending server files, credentials, databases
2. Backdoors â€” eval/exec with remote payloads, hidden commands
3. Spyware â€” logging user data secretly and sending it out
4. Credential theft â€” stealing tokens, passwords, API keys
5. Resource abuse â€” fork bombs, crypto mining
...
def _ai_scan_code(code: str, filename: str = "file.py") -> Optional[Dict[str, Any]]:
    ...
    # Limit code sent to AI â€” first 6000 chars covers most bots
...
def _combined_scan(file_path: str) -> dict:
    ...
    if ai_result is None:
        # AI unavailable â€” return pattern result as-is
        return pattern_result

    # â”€â”€ Merge AI + pattern results â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # AI says DANGEROUS â†’ always REJECT regardless of pattern score
    # AI says SAFE but pattern is DANGEROUS â†’ MANUAL_REVIEW (trust but verify)
    # AI says SUSPICIOUS â†’ at least MANUAL_REVIEW
    ...
    ai_label = f"ðŸ¤– AI ({ai_v} {ai_risk}/100): {ai_result['ai_reason']}"
    if verdict == "DANGEROUS":
        summary = f"âš ï¸ File DANGEROUS hai! {ai_label}"
    elif verdict == "SUSPICIOUS":
        summary = f"ðŸ” File suspicious hai. {ai_label}"
    else:
        summary = f"âœ… File safe hai. {ai_label}"
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• END SECURITY SCANNER â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
try:
    ...
    psutil = None  # graceful â€” used only for CPU/RAM telemetry
...
# â”‚  BOT TOKEN  add karo.   â”‚â”‚
# â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
BOT_TOKEN_HARDCODED = "8675366388:AAHOUv_JzvBTiWCSyieozvl7-CQ9cABhpOI"   # â† ADD BOT TOKEN
TOKEN = (
...
try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "2014144404"))
...
BRAND       = "â˜ï¸ á´›á´‡ÊŸá´‡Ê™á´á´› á´„ÊŸá´á´œá´… | ðŸ¸ðŸº/ðŸ½"
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = "@YourDomains"
UPDATE_CH   = "https://t.me/BABY_CODER_1"
FOOTER      = f"\n\n<blockquote>{BRAND_TAG}</blockquote>"

# â”€â”€â”€ glyphs (smart contextual symbols + emojis for the UI) â”€â”€â”€â”€â”€â”€
G = {
    ...
    "div":        "\u2501" * 16,   # â”â”â”â€¦
    "div_eq":     "\u2550" * 16,   # â•â•â•â€¦
    "div_dash":   "\u2508" * 16,   # â”ˆâ”ˆâ”ˆâ€¦
    "block_on":   "\u25A0",        # â– 
    "block_off":  "\u25A1",        # â–¡
    "border_top": "\u2550" * 16,   # â•â•â•â€¦
    "border_mid": "\u2501" * 16,   # â”â”â”â€¦
    "border_bot": "\u2550" * 16,   # â•â•â•â€¦
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wá´‡ÊŸá´„á´á´á´‡",         "#0F172A", "SÉªá´Ê€á´€É´ Há´êœ±á´›ÉªÉ´É¢"),
    "main":      ("Má´€ÉªÉ´ Má´‡É´á´œ",       "#1E1B4B", "CÊœá´á´êœ±á´‡ AÉ´ Oá´˜á´›Éªá´É´"),
    "tunnel":    ("Pá´œÊ™ÊŸÉªá´„ UÊ€ÊŸ",      "#0E7490", "CÊŸá´á´œá´…êœ°ÊŸá´€Ê€á´‡ Tá´œÉ´É´á´‡ÊŸ"),
    "bots":      ("Yá´á´œÊ€ Bá´á´›êœ±",       "#0E7490", "Má´€É´á´€É¢á´‡ & Dá´‡á´˜ÊŸá´Ê"),
    "upload":    ("Uá´˜ÊŸá´á´€á´… & Dá´‡á´˜ÊŸá´Ê", "#4338CA", "Sá´‡É´á´… Yá´á´œÊ€ FÉªÊŸá´‡êœ±"),
    "plans":     ("PÊŸá´€É´êœ± ",         "#B45309", "PÉªá´„á´‹ A TÉªá´‡Ê€"),
    "buy":       ("Bá´œÊ PÊŸá´€É´",        "#065F46", "CÊœá´‡á´„á´‹á´á´œá´›"),
    "pay":       ("Pá´€Êá´á´‡É´á´›",         "#0E7490", "Sá´‡É´á´… PÊ€á´á´êœ°"),
    "profile":   ("PÊ€á´êœ°ÉªÊŸá´‡",         "#1E3A8A", "Yá´á´œÊ€ Aá´„á´„á´á´œÉ´á´›"),
    "wallet":    ("Wá´€ÊŸÊŸá´‡á´›",          "#047857", "Tá´á´˜-Uá´˜ & Bá´€ÊŸá´€É´á´„á´‡"),
    "referral":  ("Rá´‡êœ°á´‡Ê€Ê€á´€ÊŸ",        "#9333EA", "IÉ´á´ Éªá´›á´‡ & Eá´€Ê€É´"),
    "help":      ("Há´‡ÊŸá´˜",            "#334155", "Há´á´¡ Iá´› Wá´Ê€á´‹êœ±"),
    "support":   ("Sá´œá´˜á´˜á´Ê€á´›",         "#0F766E", "Tá´€ÊŸá´‹ Tá´ Uêœ±"),
    "ticket":    ("TÉªá´„á´‹á´‡á´›êœ±",         "#0F766E", "Oá´˜á´‡É´ A TÉªá´„á´‹á´‡á´›"),
    "admin":     ("Aá´…á´ÉªÉ´ Pá´€É´á´‡ÊŸ",     "#7C2D12", "Rá´‡êœ±á´›Ê€Éªá´„á´›á´‡á´… AÊ€á´‡á´€"),
    "stats":     ("Sá´›á´€á´›êœ±",           "#14532D", "LÉªá´ á´‡ Ná´œá´Ê™á´‡Ê€êœ±"),
    "github":    ("GÉªá´›Êœá´œÊ™ Bá´€á´„á´‹á´œá´˜",   "#24292E", "SÊÉ´á´„ & Rá´‡êœ±á´›á´Ê€á´‡"),
    "security":  ("Sá´‡á´„á´œÊ€Éªá´›Ê",        "#991B1B", "Aá´œá´…Éªá´› & Ká´‡Êêœ±"),
    "bot":       ("Bá´á´› Cá´É´á´›Ê€á´ÊŸ",     "#1F2937", "Sá´›á´€Ê€á´› â€¢ Sá´›á´á´˜ â€¢ Lá´É¢êœ±"),
    "logs":      ("LÉªá´ á´‡ Lá´É¢êœ±",       "#0F172A", "Sá´›á´…á´á´œá´› / Sá´›á´…á´‡Ê€Ê€"),
    "trial":     ("FÊ€á´‡á´‡ TÊ€Éªá´€ÊŸ",      "#A21CAF", "TÊ€Ê PÊ€á´‡á´Éªá´œá´ FÊ€á´‡á´‡"),
    "coupon":    ("Cá´á´œá´˜á´É´",          "#B91C1C", "Rá´‡á´…á´‡á´‡á´ Cá´á´…á´‡"),
    "gift":      ("GÉªêœ°á´› PÊŸá´€É´",       "#9D174D", "Sá´‡É´á´… Tá´ A FÊ€Éªá´‡É´á´…"),
    "broadcast": ("BÊ€á´á´€á´…á´„á´€êœ±á´›",       "#1E40AF", "Rá´‡á´€á´„Êœ AÊŸÊŸ Uêœ±á´‡Ê€êœ±"),
    "maint":         ("Má´€ÉªÉ´á´›á´‡É´á´€É´á´„á´‡",      "#451A03", "Rá´‡á´€á´…-OÉ´ÊŸÊ Má´á´…á´‡"),
    "gh_browser":    ("GÉªá´›Êœá´œÊ™ BÊ€á´á´¡êœ±á´‡Ê€",  "#24292E", "BÊ€á´á´¡êœ±á´‡ & Rá´œÉ´"),
    "pay_config":    ("Pá´€Êá´á´‡É´á´› Cá´É´êœ°ÉªÉ¢",   "#065F46", "Rá´€á´›á´‡êœ± & Má´‡á´›Êœá´á´…êœ±"),
    "bot_config":    ("Bá´á´› Cá´É´êœ°ÉªÉ¢",        "#1F2937", "LÉªá´Éªá´›êœ± & Sá´€É´á´…Ê™á´x"),
    "appearance":    ("Aá´˜á´˜á´‡á´€Ê€á´€É´á´„á´‡",        "#4338CA", "TÊœá´‡á´á´‡ & Sá´›ÊÊŸá´‡"),
    "templates":     ("Tá´‡á´á´˜ÊŸá´€á´›á´‡êœ±",         "#0E7490", "Má´‡êœ±êœ±á´€É¢á´‡ Tá´‡á´á´˜ÊŸá´€á´›á´‡êœ±"),
    "referral_adm":  ("Rá´‡êœ°á´‡Ê€Ê€á´€ÊŸ SÊêœ±",     "#9333EA", "IÉ´á´ Éªá´›á´‡ & Eá´€Ê€É´"),
    "janitor":       ("Já´€É´Éªá´›á´Ê€",            "#451A03", "Aá´œá´›á´-CÊŸá´‡á´€É´á´œá´˜"),
    "webhooks":      ("Wá´‡Ê™Êœá´á´á´‹êœ±",          "#0F766E", "Há´á´á´‹ Má´€É´á´€É¢á´‡Ê€"),
    "features":      ("Fá´‡á´€á´›á´œÊ€á´‡ FÊŸá´€É¢êœ±",    "#B45309", "Tá´É¢É¢ÊŸá´‡ Fá´œÉ´á´„á´›Éªá´É´êœ±"),
    "monitor":       ("LÉªá´ á´‡ Má´É´Éªá´›á´Ê€",      "#14532D", "Rá´‡á´€ÊŸ-á´›Éªá´á´‡"),
    "scheduler":     ("Tá´€êœ±á´‹ Sá´„Êœá´‡á´…á´œÊŸá´‡Ê€",  "#4338CA", "Aá´œá´›á´ Tá´€êœ±á´‹êœ±"),
    "leaderboard":   ("Lá´‡á´€á´…á´‡Ê€Ê™á´á´€Ê€á´…",      "#9D174D", "Tá´á´˜ Uêœ±á´‡Ê€êœ±"),
    "subscriptions": ("Sá´œÊ™êœ±á´„Ê€Éªá´˜á´›Éªá´É´êœ±",   "#1E3A8A", "Rá´‡É´á´‡á´¡á´€ÊŸêœ±"),
    "rate_limits":   ("Rá´€á´›á´‡ LÉªá´Éªá´›êœ±",      "#991B1B", "TÊœÊ€á´á´›á´›ÊŸÉªÉ´É¢"),
    "import_export": ("Iá´á´˜á´Ê€á´› / Exá´˜á´Ê€á´›",  "#334155", "Cá´É´êœ°ÉªÉ¢ I/O"),
    "bot_controls":  ("Bá´á´› Cá´É´á´›Ê€á´ÊŸêœ±",     "#7C2D12", "Pá´‡Ê€-Bá´á´› Oá´˜êœ±"),
    "lang_panel":    ("Lá´€É´É¢á´œá´€É¢á´‡êœ±",         "#1E3A8A", "Má´œÊŸá´›Éª-Lá´€É´É¢"),
    "rev_goals":     ("Rá´‡á´ á´‡É´á´œá´‡ Gá´á´€ÊŸêœ±",    "#047857", "Tá´€Ê€É¢á´‡á´› TÊ€á´€á´„á´‹ÉªÉ´É¢"),
    "admin_2fa":     ("Adá´ÉªÉ´ 2FA",          "#991B1B", "Tá´¡á´-Fá´€á´„á´›á´Ê€ Auth"),
    "coupon_plus":   ("Cá´á´œá´˜á´É´ MÉ¢Ê€",        "#B91C1C", "Aá´…á´  Cá´á´œá´˜á´É´êœ±"),
}
...
_PHOTO_ICONS: Dict[str, str] = {
    "welcome":"âœ¦","main":"â—ˆ","tunnel":"â¬¡","bots":"â–¸","upload":"â–´",
    "plans":"â˜…","buy":"â—†","pay":"â—‰","profile":"â—ˆ","wallet":"â—†",
    "referral":"âœ¦","help":"â—‡","support":"â–«","ticket":"âœ¿","admin":"âš”",
    "stats":"â–²","github":"â¬¡","security":"â–£","bot":"â–¶","logs":"â–¸",
    "trial":"âœ¶","coupon":"â—‰","gift":"âœ¦","broadcast":"âš‘","maint":"âš™",
}


def _build_local_photos() -> None:
    """Render every banner once into storage/photos/<key>.png. Safe to
    call repeatedly â€” existing files are reused. Falls back gracefully
    ...
    for key, (text, color, sub) in _PHOTO_SPECS.items():
        # â”€â”€ Custom admin-uploaded photo takes priority over generated one â”€â”€
...
def _resolve_photo(ref: str):
    ...
    can accept. Order: cached file_id â†’ local file handle â†’ URL."""
...
_SC_MAP = str.maketrans(
    ...
    "á´€Ê™á´„á´…á´‡êœ°É¢ÊœÉªá´Šá´‹ÊŸá´É´á´á´˜QÊ€êœ±á´›á´œá´ á´¡xÊá´¢á´€Ê™á´„á´…á´‡êœ°É¢ÊœÉªá´Šá´‹ÊŸá´É´á´á´˜QÊ€êœ±á´›á´œá´ á´¡xÊá´¢",
)
...
def _atomic_write(path: Path, data: Any) -> None:
    ...
    try:
        ...
        # Cross-device or permission issue â€” fall back to copy+unlink
        try:
...
def _load_json(path: Path, default: Any) -> Any:
    ...
    try:
        ...
        # corrupt â€” keep a copy and reset
        try:
...
# â”€â”€ in-memory cache for db / settings (mtime-invalidated) â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _cached_load_ro(path: Path, default: Any) -> Any:
    ...
    paths (get_setting, is_admin, find_bot, â€¦) â€” this avoids the
...
def _cached_load(path: Path, default: Any) -> Any:
    ...
    safely without poisoning the cache. ~5-10Ã— faster than the old
    json round-trip."""
...
def db_load_ro() -> Dict[str, Any]:
    """Read-only DB access. NEVER mutate the result â€” it's the cached
...
def settings_load_ro() -> Dict[str, Any]:
    """Read-only fast path â€” DO NOT mutate."""
    with _db_lock:
...
def get_setting(key: str, default: Any = None) -> Any:
    # Hot path. Use the no-copy reader because we only `.get()` â€”
...
def cache_clear_all() -> None:
    ...
    Used by the Settings â†’ Reload button after manual file edits."""
    with _db_lock:
        _DB_CACHE.clear()
...
def is_admin(uid: int) -> bool:
    if is_owner(uid):
        return True
    # Read-only fast path â€” no deepcopy.
...
def admin_can(uid: int, action: str) -> bool:
    ...
    Permission matrix.
      owner          â†’ everything
      full-access    â†’ everything except adding admins
      manage-users   â†’ ban / give-plan / view users / approve payments / reply tickets
      view-only      â†’ view stats only
    """
    role = admin_role(uid)
    if role == "owner":
        return True
...
#  if GitHub keyring is not configured â€” see warn() below).
...
class KeyRing:
    ...
    # â”€â”€ GitHub config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    @staticmethod
    def _gh_token() -> str:
    ...
    def store(self, key_id: str, key: bytes, meta: Dict[str, Any]) -> bool:
        ...
        if not self.gh_enabled():
            # memory only â€” write a tiny encrypted local cache so a panel
    ...
    # â”€â”€ fallback local encrypted cache â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def maybe_auto_ban(uid: int, reason: str) -> None:
    ...
    if rv[str(uid)] >= 5:
        ...
        if u and not u.get("banned"):
            ...
            notify_owner(
                f"<b>{G['warn']} sá´œsá´˜Éªá´„Éªá´á´œs á´€á´„á´›Éªá´ Éªá´›Ê</b>\n\n"
...
# UI style wrapper â€” every outgoing message/caption is rendered as a
...
# â”€â”€ Flask keep-alive â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
_ka = Flask(__name__)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  9. UI HELPERS  â€”  show_menu (edit, never spam) + keyboards
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

# â”€â”€ PATCHED: ghost-delete fix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def show_menu(
    ...
    been confirmed sent â€” prevents 'ghost delete' bug."""
    cap = _html_safe_truncate(caption, 1024)

    # Any in-flight loading animation on this message is now stale â€”
    ...
    # â”€â”€ 1. Try in-place edits when the previous message is a photo â”€â”€
    if call and call.message and call.message.content_type == "photo":
        ...
        # 1b. Photo swap failed â€” keep existing photo, change only caption.
        ...
        # 1c. HTML parse blew up â€” retry caption WITHOUT parse_mode.
        try:
            plain = re.sub(r"<[^>]+>", "", cap)
            bot.edit_message_caption(
                plain,
    ...
    # â”€â”€ 2. Send a brand-new message FIRST, then delete the old one. â”€â”€
    ...
    if new_msg_id is None:
        try:
            plain = re.sub(r"<[^>]+>", "", cap)
            m = bot.send_message(
                chat_id, plain or "â€¦", reply_markup=kb,
                disable_web_page_preview=True,
            )
            new_msg_id = m.message_id
        except Exception as e:
...
def show_text(
    ...
    if new_msg_id is None:
        try:
            plain = re.sub(r"<[^>]+>", "", text)
            m = bot.send_message(chat_id, plain or "â€¦", reply_markup=kb,
                                 disable_web_page_preview=True)
            new_msg_id = m.message_id
        except Exception as e:
            _log_err("send_message(plain)", e)
...
# â”€â”€ keyboards â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"  MÊ Bá´á´›êœ±",   callback_data="menu_bots",     style="primary"),
        Btn(f" Uá´˜ÊŸá´á´€á´… Bá´á´›",   callback_data="menu_upload",   style="primary"),
    )
    kb.add(
        Btn(f"PÊŸá´€É´êœ±",        callback_data="menu_plans",    style="primary"),
        Btn(f" Bá´œÊ PÊŸá´€É´",    callback_data="menu_buy",      style="primary"),
    )
    kb.add(
        Btn(f"Rá´‡êœ°á´‡Ê€Ê€á´€ÊŸ",    callback_data="menu_referral", style="primary"),
        Btn(f"PÊ€á´êœ°ÉªÊŸá´‡",      callback_data="menu_profile",  style="primary"),
    )
    kb.add(
        Btn(f" Wá´€ÊŸÊŸá´‡á´›",     callback_data="menu_wallet",   style="primary"),
        Btn(f"TÉªá´„á´‹á´‡á´›êœ±",    callback_data="menu_tickets",  style="primary"),
    )
    kb.add(
        Btn(f" FÊ€á´‡á´‡ TÊ€Éªá´€ÊŸ",    callback_data="menu_trial",    style="primary"),
        Btn(f" Cá´á´œá´˜á´É´",        callback_data="menu_coupon",   style="primary"),
    )
    kb.add(
        Btn(f"Há´‡ÊŸá´˜",          callback_data="menu_help",     style="primary"),
        Btn(f"Sá´œá´˜á´˜á´Ê€á´›", callback_data="menu_support",  style="primary"),
    )
    kb.add(
        Btn(f" MÊ Sá´›á´€á´›êœ±",    callback_data="menu_stats",    style="primary"),
    )
    if admin:
        kb.add(Btn(f"Aá´…á´ÉªÉ´ Pá´€É´á´‡ÊŸ", callback_data="menu_admin", style="danger"))
...
def back_main_kb() -> types.InlineKeyboardMarkup:
    return types.InlineKeyboardMarkup().add(
        Btn(f"{G['back']}  Má´€ÉªÉ´ Má´‡É´á´œ", callback_data="menu_main", style="danger"))


def back_admin_kb() -> types.InlineKeyboardMarkup:
    return types.InlineKeyboardMarkup().add(
        Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def plans_kb() -> types.InlineKeyboardMarkup:
    ...
    kb.add(Btn(f"{G['back']}  Má´€ÉªÉ´ Má´‡É´á´œ", callback_data="menu_main", style="danger"))
    return kb
...
def payments_kb(plan: Optional[str] = None) -> types.InlineKeyboardMarkup:
    ...
    kb.add(Btn(f"{G['back']}  PÊŸá´€É´êœ±", callback_data="menu_plans", style="primary"))
    return kb
...
def admin_kb() -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{G['graph']}  Sá´›á´€á´›êœ±",         callback_data="adm_stats",    style="primary"),
        Btn(f"{G['users']}  Uêœ±á´‡Ê€êœ±",         callback_data="adm_users",    style="primary"),
    )
    kb.add(
        Btn(f"{G['diamond']}  AÊŸÊŸ Bá´á´›êœ±",    callback_data="adm_allbots",  style="primary"),
        Btn(f"{G['wallet']}  Pá´€Êá´á´‡É´á´›êœ±",     callback_data="adm_payments", style="success"),
    )
    kb.add(
        Btn(f"{G['broadcast']}  BÊ€á´á´€á´…á´„á´€êœ±á´›", callback_data="adm_broadcast",style="success"),
        Btn(f"{G['no']}  Bá´€É´ / UÉ´Ê™á´€É´",      callback_data="adm_ban",      style="danger"),
    )
    kb.add(
        Btn(f"{G['plus']}  GÉªá´ á´‡ PÊŸá´€É´",      callback_data="adm_giveplan", style="success"),
        Btn(f"{G['ok']}  Aá´˜á´˜Ê€á´á´ á´‡ Pá´€Ê",      callback_data="adm_approve",  style="success"),
    )
    kb.add(
        Btn(f"{G['key']}  Cá´á´œá´˜á´É´êœ±",         callback_data="adm_coupons",  style="primary"),
        Btn(f"{G['ticket']}  TÉªá´„á´‹á´‡á´›êœ±",      callback_data="adm_tickets",  style="primary"),
    )
    kb.add(
        Btn(f"{G['shield']}  Aá´…á´ÉªÉ´êœ±",       callback_data="adm_admins",   style="primary"),
        Btn(f"{G['eye']}  Aá´œá´…Éªá´› Lá´É¢",       callback_data="adm_audit",    style="primary"),
    )
    kb.add(
        Btn(f"{G['cog']}  GÉªá´›Êœá´œÊ™ Bá´€á´„á´‹á´œá´˜",   callback_data="adm_github",   style="primary"),
        Btn(f"{G['lock']}  Sá´‡á´„á´œÊ€Éªá´›Ê",       callback_data="adm_security", style="danger"),
    )
    kb.add(
        Btn(f"{G['warn']}  Má´€ÉªÉ´á´›á´‡É´á´€É´á´„á´‡",    callback_data="adm_maint",    style="danger"),
        Btn(f"{G['settings']}  Sá´‡á´›á´›ÉªÉ´É¢êœ±",   callback_data="adm_settings", style="primary"),
    )
    appr_on = bool(get_setting("approval_required", True))
    pend_n = len(get_setting("pending_uploads", {}) or {})
    kb.add(
        Btn(
            f"{G['ok'] if appr_on else G['no']}  Aá´˜á´˜Ê€á´á´ á´€ÊŸ: {'ON' if appr_on else 'OFF'}",
            callback_data="adm_approval_toggle",
            style="success" if appr_on else "danger"),
        Btn(
            f"{G['eye']}  Pá´‡É´á´…ÉªÉ´É¢" + (f" ({pend_n})" if pend_n else ""),
            callback_data="adm_pending", style="primary"),
    )
    kb.add(
        Btn(f"{G['upload']}  Má´‡É´á´œ PÊœá´á´›á´êœ±",  callback_data="adm_photos",       style="primary"),
        Btn(f"{G['refresh']}  Fá´Ê€á´„á´‡ Bá´€á´„á´‹á´œá´˜", callback_data="adm_force_backup", style="success"),
    )
    # â”€â”€ Advanced Sub-Panels Row 1 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    kb.add(
        Btn("ðŸ“Š  AÉ´á´€ÊŸÊá´›Éªá´„êœ±",       callback_data="adm_analytics",      style="primary"),
        Btn("ðŸ‘¥  Uêœ±á´‡Ê€ Tá´á´ÊŸêœ±",      callback_data="adm_user_tools",     style="primary"),
    )
    kb.add(
        Btn("ðŸ¤–  Bá´á´› Má´€É´á´€É¢á´‡Ê€",     callback_data="adm_bot_manager",    style="primary"),
        Btn("ðŸ›¡ï¸  Sá´‡á´„ Cá´‡É´á´›á´‡Ê€",      callback_data="adm_sec_center",     style="danger"),
    )
    kb.add(
        Btn("ðŸ’¬  Ná´á´›Éªêœ°Éªá´„á´€á´›Éªá´É´êœ±",   callback_data="adm_notify_center",  style="success"),
        Btn("âš™ï¸  SÊêœ± Tá´á´ÊŸêœ±",       callback_data="adm_sys_tools",      style="primary"),
    )
    # â”€â”€ MEGA ADVANCED PANELS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    kb.add(
        Btn("ðŸ™  GÊœ BÊ€á´á´¡êœ±á´‡Ê€",      callback_data="adm_gh_browser",     style="primary"),
        Btn("ðŸ’³  Pá´€Ê Cá´É´êœ°ÉªÉ¢",      callback_data="adm_pay_config",     style="success"),
    )
    kb.add(
        Btn("ðŸ”§  Bá´á´› Cá´É´êœ°ÉªÉ¢",      callback_data="adm_bot_cfg",        style="primary"),
        Btn("ðŸŽ¨  Aá´˜á´˜á´‡á´€Ê€á´€É´á´„á´‡",      callback_data="adm_appearance",     style="primary"),
    )
    kb.add(
        Btn("ðŸŽ«  Cá´á´œá´˜á´É´+",          callback_data="adm_coupon_plus",    style="primary"),
        Btn("ðŸ“  Tá´‡á´á´˜ÊŸá´€á´›á´‡êœ±",        callback_data="adm_templates",      style="primary"),
    )
    kb.add(
        Btn("ðŸ”—  Rá´‡êœ°á´‡Ê€Ê€á´€ÊŸ SÊêœ±",    callback_data="adm_referral_sys",   style="success"),
        Btn("ðŸ§¹  Já´€É´Éªá´›á´Ê€",          callback_data="adm_janitor",        style="danger"),
    )
    kb.add(
        Btn("ðŸŒ  Wá´‡Ê™Êœá´á´á´‹êœ±",         callback_data="adm_webhooks",       style="primary"),
        Btn("ðŸŽ¯  Fá´‡á´€á´›á´œÊ€á´‡ FÊŸá´€É¢êœ±",    callback_data="adm_feature_flags",  style="primary"),
    )
    kb.add(
        Btn("â±ï¸  Rá´€á´›á´‡ LÉªá´Éªá´›êœ±",      callback_data="adm_rate_config",    style="danger"),
        Btn("ðŸ“¡  LÉªá´ á´‡ Má´É´Éªá´›á´Ê€",      callback_data="adm_live_monitor",   style="success"),
    )
    kb.add(
        Btn("ðŸ’Ž  Rá´‡á´  Gá´á´€ÊŸêœ±",        callback_data="adm_rev_goals",      style="success"),
        Btn("â°  Sá´„Êœá´‡á´…á´œÊŸá´‡Ê€",         callback_data="adm_scheduler",      style="primary"),
    )
    kb.add(
        Btn("ðŸ“¥  Iá´á´˜á´Ê€á´›/Exá´˜",       callback_data="adm_import_export",  style="primary"),
        Btn("ðŸ†  Lá´‡á´€á´…á´‡Ê€Ê™á´á´€Ê€á´…",      callback_data="adm_leaderboard",    style="primary"),
    )
    kb.add(
        Btn("ðŸŒ  Lá´€É´É¢á´œá´€É¢á´‡êœ±",         callback_data="adm_languages",      style="primary"),
        Btn("ðŸ¤–  Bá´á´› Cá´É´á´›Ê€á´ÊŸêœ±",     callback_data="adm_bot_controls",   style="primary"),
    )
    kb.add(
        Btn("ðŸ‘¤  Sá´œÊ™êœ±á´„Ê€Éªá´˜á´›Éªá´É´êœ±",    callback_data="adm_subscriptions",  style="primary"),
        Btn("ðŸ”  Adá´ÉªÉ´ 2FA",         callback_data="adm_admin_2fa",      style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Má´€ÉªÉ´ Má´‡É´á´œ", callback_data="menu_main", style="primary"))
    return kb


def github_kb(status: Dict[str, Any]) -> types.InlineKeyboardMarkup:
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn(f"{G['plus']}  Bá´€á´„á´‹á´œá´˜ Ná´á´¡",      callback_data="gh_backup_now",  style="success"))
    kb.add(Btn(f"{G['refresh']}  Rá´‡êœ±á´›á´Ê€á´‡ Lá´€á´›á´‡êœ±á´›", callback_data="gh_restore_now", style="primary"))
    kb.add(Btn(
    ...
    kb.add(
        Btn(f"{G['tri']}  Sá´‡á´› BÊ€á´€É´á´„Êœ",  callback_data="gh_set_branch",   style="primary"),
        Btn(f"{G['cog']}  IÉ´á´›á´‡Ê€á´ á´€ÊŸ",    callback_data="gh_set_interval", style="primary"),
    )
    kb.add(Btn(f"{G['no']}  CÊŸá´‡á´€Ê€ Cá´É´êœ°ÉªÉ¢", callback_data="gh_clear",     style="danger"))
    kb.add(Btn(f"{G['refresh']}  Rá´‡êœ°Ê€á´‡êœ±Êœ",   callback_data="adm_github",  style="primary"))
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´",       callback_data="menu_admin",  style="primary"))
    return kb
...
def bot_actions_kb(bot_id: str, running: bool, premium: bool = False) -> types.InlineKeyboardMarkup:
    ...
    if running:
        kb.add(
            Btn(f"{G['stop']}  Sá´›á´á´˜",       callback_data=f"bot_stop_{bot_id}",    style="danger"),
            Btn(f"{G['refresh']}  Rá´‡êœ±á´›á´€Ê€á´›", callback_data=f"bot_restart_{bot_id}", style="success"),
        )
    else:
        kb.add(
            Btn(f"{G['play']}  Sá´›á´€Ê€á´›",      callback_data=f"bot_start_{bot_id}",   style="success"),
            Btn(f"{G['refresh']}  Rá´‡êœ±á´›á´€Ê€á´›", callback_data=f"bot_restart_{bot_id}", style="primary"),
        )
    kb.add(
        Btn(f"{G['bolt']}  LÉªá´ á´‡ Lá´É¢êœ±", callback_data=f"bot_logs_{bot_id}", style="primary"),
        Btn(f"{G['eye']}  IÉ´êœ°á´",       callback_data=f"bot_info_{bot_id}", style="primary"),
    )
    kb.add(
        Btn(f"{G['settings']}  EÉ´á´  Vá´€Ê€êœ±", callback_data=f"bot_env_{bot_id}",  style="primary"),
        Btn(f"{G['cog']}  CÊ€á´É´",          callback_data=f"bot_cron_{bot_id}", style="primary"),
    )
    kb.add(
        Btn(f"{G['download']}  IÉ´êœ±á´›á´€ÊŸÊŸ Pá´‹É¢", callback_data=f"bot_pip_{bot_id}",   style="primary"),
        Btn(f"{G['plus']}  CÊŸá´É´á´‡",           callback_data=f"bot_clone_{bot_id}", style="primary"),
    )
    if premium:
    ...
    kb.add(Btn(f"{G['arrow']}  Dá´á´¡É´ÊŸá´á´€á´…", callback_data=f"bot_dl_{bot_id}", style="primary"))
    kb.add(Btn(f"{G['no']}  Dá´‡ÊŸá´‡á´›á´‡",       callback_data=f"bot_delete_{bot_id}", style="danger"))
    kb.add(Btn(f"{G['back']}  MÊ Bá´á´›êœ±",    callback_data="menu_bots",            style="primary"))
    return kb
...
def detect_entry(bot_dir: Path) -> Tuple[Optional[str], Optional[str]]:
    ...
    Searches the bot dir recursively â€” many users zip their bot inside
    a wrapper folder (e.g. `MyBot/bot.py`), and the old shallow `glob`
    missed those."""
    # 1. Standard entry names â€” check shallow first, then recursive
    for n in ENTRY_NODE:
        p = bot_dir / n
        if p.exists():
            return ("node", n)
    ...
    # 4. Inner .zip â€” extract once then re-scan
...
# â”€â”€ module-name â†’ PyPI package-name mapping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# nahi chala" â€” and why uploaded bots crashed at import time.
_PYPI_ALIAS: Dict[str, str] = {
    ...
    # that does NOT expose Update / Bot â€” installing it by accident
    ...
    "github":        "PyGithub",
    "requests":      "requests",
    # extra safety net â€” pip name â‰  import name
    "nacl":          "PyNaCl",
    "git":           "GitPython",
    "jose":          "python-jose",
        ...
        for node in _ast.walk(tree):
            if isinstance(node, _ast.Import):
                ...
                if node.level and node.level > 0:
                    continue  # relative import â€” local package
                if node.module:
...
def _filter_third_party(modules: List[str], bot_dir: Path) -> List[str]:
    ...
    # any subdir â€” covers zipped wrappers like `MyBot/utils.py`)
                ...
                if _ilu.find_spec(m) is not None:
                    ...
                    if needed:
                        try:
                            ...
                            pass
                        # Wrong package â€” purge and force a reinstall.
                        try:
                            del sys.modules[m]
                        except KeyError:
                            pass
...
def _pip_env(deps_dir: Path) -> Dict[str, str]:
    ...
    NOTE: We intentionally do NOT set PYTHONUSERBASE â€” that conflicts
        ...
        if kind == "python":
            ...
            try:
                ...
                if third_party:
                    ...
                    log.append(f"ðŸ“¦ packages: {', '.join(third_party)}")
                    r2 = subprocess.run(
                        [sys.executable, "-m", "pip", "install",
                         "--target", str(deps_dir), *_PIP_BASE_FLAGS,
                         *third_party],
...
def _drain_proc(bot_id: str, proc: subprocess.Popen, log: List[str]) -> None:
    try:
        ...
        pass
    # crash-watch â€” auto-restart if plan supports it
    try:
        rc = proc.wait()
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    bid = b["_id"]
    # Approval gate â€” never start a bot still waiting for admin review.
    ...
    # â”€â”€ File-access sandbox â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # update doc â€” clear any prior crash so bot view shows clean state
    b["status"] = "running"
...
def stop_child(bot_id: str, manual: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        # 2) wait briefly â€” most well-behaved bots exit here
        try:
            proc.wait(timeout=3)
        except subprocess.TimeoutExpired:
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Cloudflared "trycloudflare" tunnels â”€
...
def _start_tunnel(bot_id: str, port: int) -> Dict[str, Any]:
    ...
    if proc.poll() is not None and rec["url"] is None:
        # process died early â€” usually port issue or network
    ...
    if rec["url"] is None:
        # No URL within 15s and process still alive â€” kill it so we don't
        ...
        tail = "\n".join(list(log_buf)[-6:]) or "(no output)"
        return {"ok": False,
                "error": f"Tunnel timed out â€” no URL after 15s.\n{tail}"}
...
def child_status(bot_id: str, b_doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "kind":      (info["kind"] if info else kind) or "â€”",
...
def store_uploaded_file(uploader: types.User, filename: str, plain: bytes) -> Dict[str, Any]:
    ...
    # notify_owner HATA DIYA â€” ab upload handler mein sirf ek summary msg aayega
...
def _make_tarball() -> Path:
    ...
    with tarfile.open(tmp, "w:gz") as tf:
        # Backup storage/ â€” users, bots DB, encrypted files, keys, tickets
        storage_dir = BASE_DIR / "storage"
        if storage_dir.exists():
            tf.add(str(storage_dir), arcname="storage", filter=_filter)
        # Backup sandbox/ â€” bot env vars, cron config (not .deps to save space)
...
_GH_UPTIME_BACKUP_THRESHOLD = 10 * 60  # seconds â€” only back up bots running >=10 min
...
def gh_auto_restore_on_boot() -> Optional[Dict[str, Any]]:
    ...
    We never overwrite a non-empty local DB â€” that would clobber any
    ...
    pulled from GitHub on boot when missing locally â€” independent of the
    DB-empty check â€” so a wiped photos folder is rebuilt on restart."""
    if not gh_enabled():
        return None
...
def _gh_sync_bot_files(b: Dict[str, Any]) -> None:
    ...
    for >=10 min â€” so broken/test uploads never reach GitHub."""
    if not gh_enabled():
        return
    try:
        _gh_ensure_branch()
...
def gh_restore_user_uploads() -> Dict[str, Any]:
    ...
    repo) â€” caller can then try the legacy tarball restore."""
    if not gh_enabled():
        return {"ok": False, "error": "Not configured."}
...
def get_or_create_user(u: types.User, ref: Optional[int] = None) -> Tuple[Dict[str, Any], bool]:
    ...
    if is_new:
        ...
        notify_owner(
            ...
            f"{bullet('Username', '@' + (u.username or 'â€”'))}\n"
...
def user_max_bots(u: Dict[str, Any]) -> int:
    ...
    # Honor admin override from Settings â†’ Plans Editor.
...
# â”€â”€ Animated progress-bar loading indicator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Active per-message animations live here so we can stop them when the
# real menu re-renders. Key: (chat_id, message_id) â†’ threading.Event.
_LOADING_STOPS: Dict[Tuple[int, int], "threading.Event"] = {}
_LOADING_LOCK = threading.Lock()


def _progress_bar(pct: int, width: int = 20) -> str:
    """`â–“â–“â–“â–“â–“â–“â–“â–“â–“â–“â–“â–“â–“â–“â–‘â–‘â–‘â–‘â–‘â–‘ 70%` style bar."""
    pct = max(0, min(100, int(pct)))
    filled = int(round(width * pct / 100))
    return "â–“" * filled + "â–‘" * (width - filled) + f" {pct:>3}%"
...
def loading(call: types.CallbackQuery, label: str = "Loading") -> None:
    """Show an animated progress bar (â–“â–“â–“â–‘â–‘â–‘ 45 %) the instant a slow
    ...
    message stops the animation automatically â€” handlers do not need
    to call anything to clean up.
    """
    if not (call and call.message):
        try:
            bot.answer_callback_query(call.id, text=f"â³ {label}â€¦")
        except Exception:
            pass
        return
    ...
    # new one (defensive â€” show_menu also cancels on re-render).
    ...
    def _animate() -> None:
        # Advance from 15% â†’ ~92% over a few seconds. We never reach
        # 100% on our own â€” the handler completing and re-rendering is
    ...
    if u and u.get("banned"):
        try:
            ...
            bot.send_message(
                ...
                f"{bullet('Reason', u.get('ban_reason') or 'â€”')}\n"
...
#   1. an "loading 10% â†’ 100%" progress bar (one message, edited live)
#   2. a CAPTCHA photo: 4 random characters, ONE has a red circle on it
#   3. inline buttons (the 4 chars + 2 distractors, shuffled) â€” user
#      must tap the *circled* one
#   4. on success â†’ user.verified = True, main menu shown
...
def _gen_captcha_image() -> Tuple[Optional[bytes], str, List[str]]:
    ...
    # background noise â€” diagonal bands
    for _ in range(10):
...
def _progress_bar_text(pct: int) -> str:
    ...
    return (
        f"<b>{G['shield']} {sc('Verifying you')}â€¦</b>\n"
        f"{G['div']}\n"
...
def _send_progress_then_captcha(chat_id: int, uid: int) -> None:
    """Phase 1: animated progress bar (one message, edited).
       Phase 2: same message edited to 'solve captcha' â€” no delete."""
    ...
    # Delete NAHI â€” edit karo
    if msg_id is not None:
        try:
            bot.edit_message_text(
                f"<b>{G['shield']} {sc('Verification loading')}â€¦ {sc('solve captcha below')} â†“</b>",
                chat_id, msg_id,
                parse_mode="HTML",
            )
        except Exception:
...
def _send_captcha(chat_id: int, uid: int) -> None:
    ...
    kb.row(
        Btn(
            f"{G.get('refresh', 'â†»')} {sc('New captcha')}",
    ...
    try:
        if png is not None:
            ...
            # PIL unavailable â€” text-only fallback
            text_cap = (
                f"<b>{G['shield']} {sc('Human verification')}</b>\n"
                f"{G['div']}\n"
...
def _verify_state_janitor() -> None:
    """Drop captcha sessions older than 10 minutes â€” prevents
...
# â”€â”€â”€ Group Join Verification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
REQUIRED_GROUPS = [
...
def _send_join_verification(chat_id: int, uid: int, not_joined: List[Dict]) -> None:
    ...
    for grp in not_joined:
        kb.add(Btn(
            f"{G['fwd']}  Já´ÉªÉ´ {grp['name']}", url=grp["link"]))
    kb.add(Btn(
        f"{G['ok']}  Vá´‡Ê€Éªêœ°Éªá´„á´€á´›Éªá´É´", callback_data="group_verify_check"))
    cap = (
...
def cb_verify(call: types.CallbackQuery) -> None:
    ...
    if not state:
        ack(call, "Session expired â€” send /start again.")
        return
    ...
    if data == state["answer"]:
        ...
        _mark_verified(uid)
        ack(call, "âœ“ Verified")
        try:
            bot.delete_message(chat_id, state["msg_id"])
        except Exception:
            pass
        intro = (
            f"<b>{G['ok']} {sc('Verification complete')}</b> â€” "
    ...
    if state["tries"] >= 3:
        ...
        try:
            ...
            pass
        ack(call, "Wrong 3 times â€” new captcha.")
        _send_captcha(chat_id, uid)
    else:
...
def render_main_menu(chat_id: int, uid: int,
    ...
    cap = (
        ...
        f"{bullet('Until', fmt_ts(u.get('plan_expires')) if u.get('plan_expires') else 'Forever' if plan['price'] == 0 else 'â€”')}\n"
...
# â”€â”€â”€ Silent mode in groups â€” bot will not respond in any group/channel â”€â”€â”€â”€â”€â”€â”€
...
def cmd_start(m: types.Message) -> None:
    ...
    if banned_block(m):
        return
    # â”€â”€ auto-claim ownership: first /start with no OWNER_ID env wins â”€â”€
    global OWNER_ID
    if OWNER_ID <= 0:
    ...
    if maintenance_block(uid):
        ...
        return
    # Human verification â€” first /start ever for this user shows a
    # progress bar (10% â†’ 100%) followed by a captcha photo. Once the
    # captcha is solved, render_main_menu is called from cb_verify.
    if not require_verified(m.chat.id, uid):
        return

    # Group join verification â€” user must join required groups
...
def cmd_help(m: types.Message) -> None:
    ...
    if not require_verified(m.chat.id, m.from_user.id):
        return
    txt = (
        f"<b>{esc(BRAND_TAG)} â€” {sc('Quick Help')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Upload',  'Send a .py / .js / .zip file or use Upload Bot menu.')}\n"
        f"{bullet('Manage',  'My Bots â†’ pick a bot â†’ Start / Stop / Logs.')}\n"
        f"{bullet('Plans',   'Plans â†’ Buy Plan â†’ choose method â†’ send proof.')}\n"
...
# â”€â”€â”€ callback de-duplication â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cb_root(call: types.CallbackQuery) -> None:
    ...
    if maintenance_block(uid):
        ...
        return
    # Block menu navigation for unverified users â€” they must solve the
    ...
    if not _is_verified(uid):
        ack(call, "Please solve the captcha first â€” send /start.")
        return
    data = call.data or ""
    try:
        ...
        traceback.print_exc()
        try:
            bot.send_message(call.message.chat.id, f"<b>{G['no']}</b> EÊ€Ê€á´Ê€: <code>{esc(e)}</code>")
        except Exception:
            pass


def _route_callback(call: types.CallbackQuery, data: str) -> None:
    # â”€â”€â”€ core menu navigation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if data == "menu_main":
    ...
    # â”€â”€â”€ plan view + buy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ pay methods â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if data.startswith("pay_"):
        ack(call); render_payment_screen(call, data); return
    if data == "pay_proof":
        ack(call); start_proof_flow(call); return

    # â”€â”€â”€ bot actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ approval system (admin only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ admin sub-actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ trial â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if data == "trial_claim":
        ack(call); action_trial_claim(call); return

    # â”€â”€â”€ coupon redeem â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ wallet top-up request â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if data == "wallet_topup":
        ack(call); start_wallet_topup(call); return
    if data == "wallet_gift":
        ack(call); start_wallet_gift(call); return

    # â”€â”€â”€ admin payment approve/reject â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€â”€ unknown â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ack(call, "?")
...
def render_upload_menu(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"All files are <b>encrypted at rest</b> with Fernet/AES-128 â€” keys live in our private key vault."
    )
...
def render_payment_screen(call: types.CallbackQuery, data: str) -> None:
    ...
    cap = (
        f"<b>{pm['tag']} {esc(pm['name'])} â€” {sc('Payment')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_profile(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Username', '@' + (u.get('username') or 'â€”'))}\n"
        f"{bullet('User ID',  uid)}\n"
        f"{bullet('Plan',     p['name'])}\n"
        f"{bullet('Until',    fmt_ts(u.get('plan_expires')) if u.get('plan_expires') else ('Forever' if p['price'] == 0 else 'â€”'))}\n"
        f"{bullet('Wallet',   '{}$'.format(u.get('wallet', 0)))}\n"
...
def render_help(call: types.CallbackQuery) -> None:
    cap = (
        ...
        f"{bullet('Run',     'My Bots â†’ pick â†’ Start')}\n"
        f"{bullet('Logs',    'My Bots â†’ pick â†’ Live Logs')}\n"
        f"{bullet('Env',     'My Bots â†’ pick â†’ Env Vars')}\n"
        f"{bullet('Plans',   'Plans â†’ Buy Plan â†’ method')}\n"
        f"{bullet('Coupon',  'Coupon menu â†’ Redeem')}\n"
...
def render_trial(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{sc('Get a free 48-hour Pro trial â€” one time per account')}.\n"
...
def render_user_stats(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Last Payment',   fmt_ts(last_pay) if last_pay else 'â€”')}\n"
...
def start_coupon_flow(call: types.CallbackQuery) -> None:
    ...
    bot.send_message(
        call.message.chat.id,
        f"{G['key']} {sc('Send your coupon code')} (Tá´‡xá´› OÉ´ÊŸÊ). /cancel {sc('to abort')}.",
    )
...
def render_bot_view(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (
        ...
        f"{bullet('Kind',    st['kind'] or 'â€”')}\n"
        f"{bullet('PID',     'â€¢â€¢â€¢â€¢' if st['pid'] else 'â€”')}\n"
...
def action_bot_logs(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    txt = (
        f"<b>{G['bolt']} {sc('Live Logs')} â€” {esc(b['name'])}</b>\n"
        f"{G['div_eq']}\n<pre>"
        + esc("\n".join(last))[:3500]
        + f"</pre>\n{G['div']}{FOOTER}"
    )
...
def render_bot_delete_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (
        ...
        f"{G['bullet']} <b>{sc('Delete Bot Files')}</b> â€” {sc('removes files and keys only')}\n"
        f"{G['bullet']} <b>{sc('Delete All Data')}</b> â€” {sc('removes files keys AND GitHub backup')}\n\n"
...
def render_bot_delfiles_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b:
        ack(call, "Not found"); return
    cap = (
        f"<b>{G['trash']} {sc('Delete Bot Files')} â€” {esc(b['name'])}</b>\n"
        f"{G['div_eq']}\n"
...
def render_bot_delall_confirm(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if not b:
        ack(call, "Not found"); return
    cap = (
        f"<b>{G['no']} {sc('Delete All Data')} â€” {esc(b['name'])}</b>\n"
        f"{G['div_eq']}\n"
    ...
    try:
        ...
        with open(out, "rb") as fh:
            bot.send_document(
                ...
                caption=f"{G['download']} {sc('Bot files')} â€” {esc(b['name'])}",
...
def render_env_menu(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (
        f"<b>{G['settings']} {sc('Env Vars')} â€” {esc(b['name'])}</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup()
    kb.add(Btn(
...
def _handle_tunnel_port(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    if not res.get("ok"):
        try:
            ...
            pass
        return
    url = res.get("url") or "(provisioningâ€¦)"
    try:
        bot.edit_message_text(
...
def render_cron(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cron = b.get("cron") or {}
    cap = (
        f"<b>{G['cog']} {sc('Cron')} â€” {esc(b['name'])}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Restart every', cron.get('restart_hours', 'â€”'))}\n"
        f"{bullet('Backup every',  cron.get('backup_hours', 'â€”'))}\n"
        f"{G['div']}\n"
...
def render_admin_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    # â”€â”€ advanced settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if data.startswith("adm_set_plan_show_"):
        ack(call, "Use âž• / âž– to adjust"); return
    ...
    if data == "adm_set_reload":
        ...
        ack(call, "Caches dropped â€” next read = disk")
        return render_adm_settings(call)
    ...
    # â”€â”€ NEW: 6 Advanced Sub-Panel Routes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• MEGA ADVANCED PANEL ROUTES â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # GitHub Browser
    ...
    if data == "adm_rebuild_banners":
        ...
        ack(call, f"{G['ok']} Banner cache cleared â€” photos will reload fresh")
    ...
    if data == "adm_bc_set_currency_symbol":
        ...
        bot.send_message(call.message.chat.id, f"{G['settings']} Send new currency symbol (e.g. â‚¹ $ â‚¬):", parse_mode="HTML"); return
    ...
    if data.startswith("adm_tmpl_edit_"):
        ...
        bot.send_message(call.message.chat.id,
                         f"<b>ðŸ“ {sc('Edit Template')}: <code>{esc(key)}</code></b>\n"
                         f"{G['div']}\n<i>{sc('Current')}:</i>\n{esc(cur) or '(default)'}\n\n"
    ...
    if data == "adm_ref_set_reward":
        ...
        bot.send_message(call.message.chat.id,
                         f"{G['settings']} {sc('Send wallet reward amount per referral (in à§³)')}."); return
    ...
    if data == "adm_wh_clear":
        try:
            ...
            ack(call, "Webhook cleared â†’ polling mode")
    ...
    if data == "adm_goal_set_monthly":
        ...
        bot.send_message(call.message.chat.id, f"{G['settings']} {sc('Send monthly revenue target (à§³)')}:"); return
    if data == "adm_goal_set_yearly":
        USER_STATES[call.from_user.id] = {"flow": "await_adm_goal_set", "goal_type": "yearly"}
        bot.send_message(call.message.chat.id, f"{G['settings']} {sc('Send yearly revenue target (à§³)')}:"); return
    ...
    if data == "adm_import_reset":
        ...
        bot.send_message(call.message.chat.id,
                         f"âš ï¸ <b>{sc('FACTORY RESET')}</b> â€” {sc('Type')} <code>CONFIRM RESET</code> "
    ...
    if data.startswith("adm_bc_restart_"):
        ...
        if b:
            ...
            ack(call, f"Restarting {b.get('name','?')[:15]}â€¦")
        return
    ...
    if data == "adm_sub_run_downgrade":
        if not is_owner(call.from_user.id): ack(call, "Owner only"); return
        ack(call, "Running downgrade nowâ€¦")
...
def render_adm_users(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{G['bullet']} <code>{u['_id']}</code> â€” {esc(u.get('name'))} "
        f"(@{esc(u.get('username') or 'â€”')}) "
...
def render_adm_allbots(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{G['bullet']} <code>{b['_id']}</code> â€” {esc(b['name'])} "
...
def render_adm_payments(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"{G['bullet']} {esc(p.get('plan', 'â€”'))} {G['bullet']} {esc(p.get('method'))}"
        for p in pays
...
def render_adm_broadcast(call: types.CallbackQuery) -> None:
    cap = (
        ...
        f"  <code>plan:pro</code> â€” {sc('only pro users')}\n"
        f"  <code>plan:free</code> â€” {sc('only free users')}\n"
        f"  <code>at:YYYY-MM-DD HH:MM</code> â€” {sc('schedule')}\n"
...
def render_adm_coupons(call: types.CallbackQuery) -> None:
    d = db_load()["coupons"]
    rows = "\n".join(
        f"{G['bullet']} <code>{esc(code)}</code> â€” {esc(c.get('percent'))}% "
...
def render_adm_tickets(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{G['bullet']} <code>{t['id']}</code> uid {t['uid']} â€” {esc(t.get('subject'))[:40]}"
        for t in open_t
...
def render_adm_admins(call: types.CallbackQuery) -> None:
    ...
    d = db_load()["admins"]
    rows = "\n".join(
        f"{G['bullet']} <code>{uid}</code> â€” {esc(a.get('role'))}"
...
def render_adm_audit(call: types.CallbackQuery) -> None:
    d = db_load()["audit"][-25:]
    rows = "\n".join(
        f"{G['bullet']} {esc(a['ts'][11:19])} uid {a['uid']} â†’ {esc(a['action'])} {esc(a.get('detail', ''))[:60]}"
        for a in reversed(d)
...
def render_adm_pending(call: types.CallbackQuery) -> None:
    ...
    if not items:
        cap = (
            ...
            f"{G['div_eq']}\n<i>{sc('Inbox is empty â€” nothing waiting for approval')}.</i>\n"
            f"{G['div']}{FOOTER}"
        )
        show_menu(call.message.chat.id, PHOTOS["admin"], cap, back_admin_kb(), call=call)
        return
    ...
    for bid, info in items[:15]:
        ...
        rows.append(
            f"{G['bullet']} <code>{esc(bid)}</code> â€” {esc(nm)} "
...
def render_adm_github(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Last err',   s['lastError'] or 'â€”')}\n"
        f"{G['div']}{FOOTER}"
    )
...
def render_github_subroute(call: types.CallbackQuery, data: str) -> None:
    ...
    if data == "gh_set_token":
        ...
        bot.send_message(call.message.chat.id, f"{G['key']} {sc('Send the GitHub token now')} (Tá´‡xá´›)."); return
    if data == "gh_set_repo":
        USER_STATES[call.from_user.id] = {"flow": "await_gh_repo"}
        bot.send_message(call.message.chat.id, f"{G['diamond']} {sc('Send the repo as')} <code>Oá´¡É´á´‡Ê€/repo</code>.", parse_mode="HTML"); return
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{bullet('Announce chan',  ANNOUNCE_CHANNEL or 'â€”')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    # â”€â”€ live tunables â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    kb.add(
    ...
    # â”€â”€ ops actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    kb.add(
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  Advanced settings â€” sub-renderers + handlers
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_sysinfo(call: types.CallbackQuery) -> None:
    """Live system info â€” RAM, disk, uptime, child processes."""
    rss = vms = pct = 0
    if psutil is not None:
        try:
...
def render_adm_plans(call: types.CallbackQuery) -> None:
    """Live plan editor â€” adjust max_bots per plan tier."""
    rows = []
    for k, v in PLAN_LIMITS.items():
...
# â”€â”€â”€ 1. ANALYTICS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_analytics(call: types.CallbackQuery) -> None:
    ...
    running_n = sum(1 for x in RUNNING.values() if x["proc"].poll() is None)
    cap = (
        f"<b>ðŸ“Š {sc('Analytics Dashboard')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total Revenue',   f'{total_rev}à§³')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“ˆ  Rá´‡á´ á´‡É´á´œá´‡ Rá´‡á´˜á´Ê€á´›",  callback_data="adm_revenue_report", style="success"),
        Btn("ðŸ“‰  GÊ€á´á´¡á´›Êœ Sá´›á´€á´›êœ±",    callback_data="adm_growth_stats",   style="primary"),
    )
    kb.add(
        Btn("ðŸ†  Tá´á´˜ Uêœ±á´‡Ê€êœ±",       callback_data="adm_top_users",      style="primary"),
        Btn("ðŸ¥§  PÊŸá´€É´ DÉªêœ±á´›",       callback_data="adm_plan_dist",      style="primary"),
    )
    kb.add(
        Btn("ðŸ¤–  Bá´á´› Aá´„á´›Éªá´ Éªá´›Ê",   callback_data="adm_bot_activity",   style="primary"),
        Btn("ðŸ“Š  Sá´›á´€á´›êœ± Oá´ á´‡Ê€á´ Éªá´‡á´¡",  callback_data="adm_stats",          style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_revenue_report(call: types.CallbackQuery) -> None:
    ...
    by_plan = "\n".join(f"{bullet(k, f'{v}à§³')}" for k, v in sorted(plan_rev.items()))
    cap = (
        f"<b>ðŸ“ˆ {sc('Revenue Report')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Today',        f'{rev_day}à§³')}\n"
        f"{bullet('Last 7 days',  f'{rev_week}à§³')}\n"
        f"{bullet('Last 30 days', f'{rev_month}à§³')}\n"
        f"{bullet('All time',     f'{rev_all}à§³')}\n"
        f"{G['div']}\n<b>{sc('By Plan')}:</b>\n{by_plan}{FOOTER}"
    )
...
def render_adm_growth_stats(call: types.CallbackQuery) -> None:
    ...
    bar = lambda n, mx: "â–ˆ" * int(n / max(mx, 1) * 10) + "â–‘" * (10 - int(n / max(mx, 1) * 10))
    d1, d7, d30, all_ = _count(1), _count(7), _count(30), len(list(users))
    cap = (
        f"<b>ðŸ“‰ {sc('User Growth')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_top_users(call: types.CallbackQuery) -> None:
    ...
    for i, (uid, amt) in enumerate(top, 1):
        ...
        rows.append(f"{i}. {name} â€” <b>{amt}à§³</b> {G['bullet']} {bot_count} bots")
    cap = (
        f"<b>ðŸ† {sc('Top Users by Spending')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_bot_activity(call: types.CallbackQuery) -> None:
    ...
    bar = lambda n: "â–ˆ" * int(n / max(total, 1) * 10)
    cap = (
        f"<b>ðŸ¤– {sc('Bot Activity')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total bots',   total)}\n"
        f"{bullet('â–¶ Running',    f'{running}  {bar(running)}')}\n"
        f"{bullet('â¹ Stopped',    f'{stopped}  {bar(stopped)}')}\n"
        f"{bullet('ðŸ’¥ Crashed',   f'{crashed}  {bar(crashed)}')}\n"
        f"{bullet('â¬œ Never run', never)}\n"
        f"{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, _adm_back("adm_analytics"), call=call)


# â”€â”€â”€ 2. USER TOOLS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def render_adm_user_tools(call: types.CallbackQuery) -> None:
    d = db_load()
    banned_n = sum(1 for u in d["users"].values() if u.get("banned"))
    cap = (
        f"<b>ðŸ‘¥ {sc('User Tools')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total users', len(d['users']))}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ”  Sá´‡á´€Ê€á´„Êœ Uêœ±á´‡Ê€",    callback_data="adm_user_search",     style="primary"),
        Btn("ðŸš«  Bá´€É´É´á´‡á´… LÉªêœ±á´›",    callback_data="adm_banned_list",     style="danger"),
    )
    kb.add(
        Btn("ðŸ’°  Wá´€ÊŸÊŸá´‡á´› Aá´…á´Šá´œêœ±á´›",  callback_data="adm_wallet_admin",    style="success"),
        Btn("ðŸ“¤  Exá´˜á´Ê€á´› CSV",      callback_data="adm_user_export_csv", style="primary"),
    )
    kb.add(
        Btn("ðŸ“¨  Ná´á´›Éªêœ°Ê Uêœ±á´‡Ê€",    callback_data="adm_notify_user",     style="primary"),
        Btn("ðŸ”„  Rá´‡êœ±á´‡á´› Uêœ±á´‡Ê€",     callback_data="adm_user_reset",      style="danger"),
    )
    kb.add(
        Btn("ðŸŽ  GÉªá´ á´‡ PÊŸá´€É´",      callback_data="adm_giveplan",        style="success"),
        Btn("ðŸš«  Bá´€É´/UÉ´Ê™á´€É´",      callback_data="adm_ban",             style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_banned_list(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{G['bullet']} <code>{uid}</code> â€” {esc(u.get('name','?'))} "
        f"({esc(u.get('ban_reason','â€”'))})"
        for uid, u in banned[:20]
    ) or f"<i>{sc('No banned users')}</i>"
    cap = (
        f"<b>ðŸš« {sc('Banned Users')} ({len(banned)})</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, _adm_back("adm_user_tools"), call=call)
...
def render_adm_wallet_admin(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ’° {sc('Adjust User Wallet')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_notify_user(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ“¨ {sc('Notify Specific User')}</b>\n"
        f"{G['div_eq']}\n"
...
# â”€â”€â”€ 3. BOT MANAGER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_bot_manager(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ’¥  CÊ€á´€êœ±Êœá´‡á´… Bá´á´›êœ±",     callback_data="adm_crashed_bots",        style="danger"),
        Btn("ðŸ”„  Rá´‡êœ±á´›á´€Ê€á´› Sá´›á´á´˜á´˜á´‡á´…",   callback_data="adm_mass_restart_stopped", style="success"),
    )
    kb.add(
        Btn("ðŸ”  Sá´‡á´€Ê€á´„Êœ Bá´á´›",        callback_data="adm_bot_search",          style="primary"),
        Btn("ðŸ“¦  SÉªá´¢á´‡ Rá´‡á´˜á´Ê€á´›",       callback_data="adm_bot_size_report",     style="primary"),
    )
    kb.add(
        Btn("ðŸ§ª  AI Sá´„á´€É´ Pá´‡É´á´…ÉªÉ´É¢",   callback_data="adm_force_scan_all",      style="primary"),
        Btn("ðŸ“‹  AÊŸÊŸ Bá´á´›êœ±",          callback_data="adm_allbots",             style="primary"),
    )
    kb.add(
        Btn("ðŸ”´  KÉªÊŸÊŸ AÊŸÊŸ Ná´á´¡",     callback_data="adm_kill_all_now",        style="danger"),
        Btn("ðŸ—‘ï¸  CÊŸá´‡á´€É´ OÊ€á´˜Êœá´€É´êœ±",    callback_data="adm_set_clean_orphans",   style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_crashed_bots(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"â€” exit <b>{b.get('last_exit_code')}</b> "
        f"uid {b.get('owner')}"
        for b in crashed[:20]
    ) or f"<i>{sc('No crashed bots')}</i>"
    cap = (
        f"<b>ðŸ’¥ {sc('Crashed Bots')} ({len(crashed)})</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
...
def render_adm_mass_restart_stopped(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ”„ {sc('Mass Restart Stopped Bots')}</b>\n"
        f"{G['div_eq']}\n"
...
def action_adm_force_scan_all(call: types.CallbackQuery) -> None:
    ack(call, "Scanning pending bots with AIâ€¦")
    def _bg() -> None:
        ...
        for bid, info in pending[:5]:
            ...
            try:
                ...
                if verdict in ("DANGEROUS", "SUSPICIOUS"):
                    flagged += 1
                    results.append(f"âš ï¸ {b['name'][:20]}: {verdict}")
                else:
                    results.append(f"âœ… {b['name'][:20]}: SAFE")
            except Exception as e:
                results.append(f"âŒ {bid[:8]}: error")
        summary = "\n".join(results) or "No pending bots to scan."
        audit(call.from_user.id, "force_scan_all", f"scanned={scanned} flagged={flagged}")
        try:
            bot.send_message(
                call.from_user.id,
                f"<b>ðŸ§ª {sc('AI Scan Report')}</b>\n"
                f"{G['div_eq']}\n"
...
# â”€â”€â”€ 4. SECURITY CENTER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_sec_center(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“‹  TÊœÊ€á´‡á´€á´› Lá´É¢",      callback_data="adm_threat_log",     style="danger"),
        Btn("ðŸ“Š  Sá´‡á´„ Sá´›á´€á´›êœ±",        callback_data="adm_sec_stats",      style="primary"),
    )
    kb.add(
        Btn("âœ…  WÊœÉªá´›á´‡ÊŸÉªêœ±á´› Uêœ±á´‡Ê€",  callback_data="adm_sec_whitelist",  style="success"),
        Btn("ðŸš«  BÊŸá´€á´„á´‹ÊŸÉªêœ±á´›",        callback_data="adm_sec_blacklist",  style="danger"),
    )
    kb.add(
        Btn("ðŸ”  Sá´„á´€É´ Rá´‡á´˜á´Ê€á´›",     callback_data="adm_scan_report",    style="primary"),
        Btn("ðŸš«  Bá´€É´É´á´‡á´… LÉªêœ±á´›",     callback_data="adm_banned_list",    style="primary"),
    )
    kb.add(
        Btn("ðŸ›¡ï¸  Sá´‡á´„á´œÊ€Éªá´›Ê IÉ´êœ°á´",   callback_data="adm_security",       style="primary"),
        Btn("ðŸ“‹  Aá´œá´…Éªá´› Lá´É¢",        callback_data="adm_audit",          style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_sec_stats(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ“Š {sc('Security Statistics')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total scans',    total)}\n"
        f"{bullet('ðŸ”´ Blocked',     blocked)}\n"
        f"{bullet('ðŸŸ¡ Suspicious',  sus)}\n"
        f"{bullet('âœ… Safe',        safe_n)}\n"
...
def render_adm_scan_report(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ” {sc('Recent Scan Report')} (last {len(last10)})</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS["security"], cap, _adm_back("adm_sec_center"), call=call)
...
# â”€â”€â”€ 5. NOTIFICATIONS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def render_adm_notify_center(call: types.CallbackQuery) -> None:
    users_n  = len(db_load()["users"])
    running_n = sum(1 for x in RUNNING.values() if x["proc"].poll() is None)
    cap = (
        f"<b>ðŸ’¬ {sc('Notifications Center')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“¢  Ná´á´›Éªêœ°Ê Eá´ á´‡Ê€Êá´É´á´‡", callback_data="adm_notify_all",        style="success"),
        Btn("â–¶ï¸  Bá´á´› Uêœ±á´‡Ê€êœ± OÉ´ÊŸÊ",  callback_data="adm_notify_running",     style="primary"),
    )
    kb.add(
        Btn("ðŸ“Š  BÊ PÊŸá´€É´",          callback_data="adm_notify_plan_select", style="primary"),
        Btn("ðŸ“¨  SÉªÉ´É¢ÊŸá´‡ Uêœ±á´‡Ê€",     callback_data="adm_notify_user",        style="primary"),
    )
    kb.add(
        Btn("â°  Sá´„Êœá´‡á´…á´œÊŸá´‡ Mêœ±É¢",    callback_data="adm_schedule_msg",       style="primary"),
        Btn("ðŸ“£  Qá´œÉªá´„á´‹ AÉ´É´á´á´œÉ´á´„á´‡",  callback_data="adm_quick_announce",     style="success"),
    )
    kb.add(
        Btn("ðŸ“¡  BÊ€á´á´€á´…á´„á´€êœ±á´›",        callback_data="adm_broadcast",          style="success"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_notify_all(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ“¢ {sc('Notify All Users')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Recipients', total)}\n"
        f"{sc('Send your message now â€” it will be delivered to every user')}.{FOOTER}"
    )
    USER_STATES[call.from_user.id] = {"flow": "await_broadcast"}
...
def render_adm_notify_running(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>â–¶ï¸ {sc('Notify Active Bot Users')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_notify_plan(call: types.CallbackQuery, plan_key: str) -> None:
    ...
    cap = (
        f"<b>ðŸ“Š {sc('Notify')} {esc(plan_name)} {sc('Users')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_quick_announce(call: types.CallbackQuery) -> None:
    chan = ANNOUNCE_CHANNEL or "â€”"
    cap = (
        f"<b>ðŸ“£ {sc('Quick Announce')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Channel', chan)}\n"
        f"{sc('Send your message â€” it will be pinned in the announce channel')}.{FOOTER}"
    )
    USER_STATES[call.from_user.id] = {"flow": "await_adm_quick_announce"}
    show_menu(call.message.chat.id, PHOTOS["broadcast"], cap, _adm_back("adm_notify_center"), call=call)


# â”€â”€â”€ 6. SYSTEM TOOLS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_sys_tools(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ–¥ï¸  SÊêœ± Há´‡á´€ÊŸá´›Êœ",    callback_data="adm_sys_health",      style="primary"),
        Btn("ðŸ’¾  DÉªêœ±á´‹ Uêœ±á´€É¢á´‡",    callback_data="adm_disk_usage",      style="primary"),
    )
    kb.add(
        Btn("ðŸ—„ï¸  DB IÉ´êœ°á´",       callback_data="adm_db_info",         style="primary"),
        Btn("ðŸ§¹  CÊŸá´‡á´€Ê€ Cá´€á´„Êœá´‡",   callback_data="adm_clear_cache",     style="danger"),
    )
    kb.add(
        Btn("ðŸ”‘  Tá´á´‹á´‡É´ CÊœá´‡á´„á´‹",   callback_data="adm_token_check",     style="primary"),
        Btn("ðŸ“¤  Exá´˜á´Ê€á´› Dá´€á´›á´€",   callback_data="adm_set_export",      style="primary"),
    )
    kb.add(
        Btn("ðŸ”„  Rá´‡ÊŸá´á´€á´… Cá´€á´„Êœá´‡",  callback_data="adm_set_reload",      style="success"),
        Btn("ðŸ‘ï¸  SÊêœ±á´›á´‡á´ IÉ´êœ°á´",   callback_data="adm_set_sysinfo",     style="primary"),
    )
    kb.add(
        Btn("âœï¸  Fá´á´á´›á´‡Ê€ Tá´‡xá´›",   callback_data="adm_set_footer_text", style="primary"),
        Btn("ðŸ‘‹  Wá´‡ÊŸá´„á´á´á´‡ Mêœ±É¢",   callback_data="adm_set_welcome_text",style="primary"),
    )
    kb.add(
        Btn("ðŸ“œ  Rá´œÊŸá´‡êœ± Tá´‡xá´›",    callback_data="adm_set_rules_text",  style="primary"),
        Btn("ðŸ“¦  GÉªá´›Êœá´œÊ™",         callback_data="adm_github",          style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, kb, call=call)
...
def render_adm_disk_usage(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"({esc(d['users'].get(uid, {}).get('name', '?')[:15])}) â€” <b>{fmt_bytes(sz)}</b>"
        for uid, sz in top
    ) or f"<i>{sc('No data')}</i>"
    cap = (
        f"<b>ðŸ’¾ {sc('Disk Usage by User')}</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS["admin"], cap, _adm_back("adm_sys_tools"), call=call)
    ...
    def _bg() -> None:
        ...
        for b in bots[:20]:
            ...
            try:
                ...
                invalid += 1
                bad_list.append(b.get("name", b["_id"])[:20])
        bad_txt = "\n".join(f"  âŒ {n}" for n in bad_list) or "  (none)"
        audit(call.from_user.id, "token_check", f"valid={valid} invalid={invalid}")
        try:
            bot.send_message(
                call.from_user.id,
                f"<b>ðŸ”‘ {sc('Token Check Report')}</b>\n"
                f"{G['div_eq']}\n"
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• END NEW ADMIN SUB-PANELS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•


# â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
# â•‘          MEGA ADVANCED ADMIN PANELS  (20+ new panels, 200+ features)     â•‘
...
_MESSAGE_TEMPLATES: Dict[str, Dict[str, str]] = {
    "welcome": {
        ...
        "default": "Welcome {name}! ðŸŽ‰ You're now registered on {brand}. Use /start to explore.",
        "vars": "{name}, {brand}, {plan}",
    },
    "payment_received": {
        "label": "Payment Received",
        "default": "âœ… Payment of {amount}à§³ received for {plan} plan. Your account has been upgraded!",
        "vars": "{name}, {amount}, {plan}, {tx_id}, {date}",
    },
    "plan_expired": {
        "label": "Plan Expiry Warning",
        "default": "âš ï¸ Your {plan} plan expires in {days} days. Renew now to avoid service interruption!",
        "vars": "{name}, {plan}, {days}, {expiry_date}",
    },
    "bot_approved": {
        "label": "Bot Approved",
        "default": "âœ… Your bot '{bot_name}' has been approved and is now running!",
        "vars": "{name}, {bot_name}, {bot_id}",
    },
    "bot_rejected": {
        "label": "Bot Rejected",
        "default": "âŒ Your bot '{bot_name}' was rejected. Reason: {reason}",
        "vars": "{name}, {bot_name}, {reason}",
    },
    "referral_reward": {
        "label": "Referral Reward",
        "default": "ðŸŽ You earned {amount}à§³ for referring {referred_name}! Keep sharing!",
        "vars": "{name}, {amount}, {referred_name}",
    },
    "ticket_reply": {
        "label": "Ticket Reply",
        "default": "ðŸ“© Admin replied to your ticket #{ticket_id}: {reply}",
        "vars": "{name}, {ticket_id}, {reply}",
    },
    "bot_crashed": {
        "label": "Bot Crashed Alert",
        "default": "ðŸ’¥ Your bot '{bot_name}' crashed (exit code {exit_code}). Check logs or re-upload.",
        "vars": "{name}, {bot_name}, {exit_code}",
    },
    "maintenance": {
        "label": "Maintenance Notice",
        "default": "ðŸ”§ {brand} is currently under maintenance. We'll be back soon!",
        "vars": "{brand}, {eta}",
    },
    "upgrade_prompt": {
        "label": "Upgrade Prompt",
        "default": "ðŸ’Ž Upgrade to {plan} and get {max_bots} bots, {ram}MB RAM, and more!",
...
_SUPPORTED_LANGUAGES: Dict[str, str] = {
    ...
    "fa":    "ðŸ‡®ðŸ‡· ÙØ§Ø±Ø³ÛŒ (Persian)",
    "zh":    "ðŸ‡¨ðŸ‡³ ä¸­æ–‡ (Chinese)",
}

_APPEARANCE_THEMES: Dict[str, Dict[str, str]] = {
    "dark":      {"name": "Dark",       "header": "#0F172A", "accent": "#6366F1", "emoji_ok": "âœ…"},
    "midnight":  {"name": "Midnight",   "header": "#020617", "accent": "#818CF8", "emoji_ok": "ðŸ’«"},
    "ocean":     {"name": "Ocean",      "header": "#0E4472", "accent": "#38BDF8", "emoji_ok": "ðŸŒŠ"},
    "forest":    {"name": "Forest",     "header": "#14532D", "accent": "#4ADE80", "emoji_ok": "ðŸŒ¿"},
    "sunset":    {"name": "Sunset",     "header": "#7C2D12", "accent": "#FB923C", "emoji_ok": "ðŸŒ…"},
    "royal":     {"name": "Royal",      "header": "#3B0764", "accent": "#C084FC", "emoji_ok": "ðŸ‘‘"},
    "neon":      {"name": "Neon",       "header": "#0A0A0A", "accent": "#39FF14", "emoji_ok": "âš¡"},
    "rose":      {"name": "Rose",       "header": "#881337", "accent": "#FB7185", "emoji_ok": "ðŸŒ¹"},
    "gold":      {"name": "Gold",       "header": "#451A03", "accent": "#FBBF24", "emoji_ok": "ðŸ’°"},
    "ice":       {"name": "Ice",        "header": "#1E3A5F", "accent": "#BAE6FD", "emoji_ok": "â„ï¸"},
}
...
def render_adm_gh_browser(call: types.CallbackQuery) -> None:
    """GitHub File Browser â€” main landing panel."""
    has_token = bool(GH.get("token"))
    has_repo  = bool(GH.get("repo"))
    cur_repo  = GH.get("repo") or "â€”"
    cur_branch= GH.get("branch") or "main"
    cap = (
        f"<b>ðŸ™ {sc('GitHub File Browser')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Token',  'âœ… Set' if has_token else 'âŒ Not set')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    if has_token:
        kb.add(Btn("ðŸ“‚  BÊ€á´á´¡êœ±á´‡ MÊ Rá´‡á´˜á´êœ±",  callback_data="adm_gh_repos",        style="success"))
        if has_repo:
            kb.add(Btn(f"ðŸ“  {esc(cur_repo)[:25]}",  callback_data="adm_gh_browse_repo", style="primary"))
    kb.add(
        Btn("ðŸ”‘  Sá´‡á´› Tá´á´‹á´‡É´",       callback_data="gh_set_token",   style="primary"),
        Btn("ðŸ“¦  Sá´‡á´› Rá´‡á´˜á´",         callback_data="gh_set_repo",    style="primary"),
    )
    kb.add(
        Btn("ðŸŒ¿  Sá´‡á´› BÊ€á´€É´á´„Êœ",       callback_data="gh_set_branch",  style="primary"),
        Btn("ðŸ”„  Rá´‡êœ°Ê€á´‡êœ±Êœ",          callback_data="adm_gh_refresh_repos", style="primary"),
    )
    kb.add(
        Btn("ðŸ“Š  GÉªá´›Êœá´œÊ™ Bá´€á´„á´‹á´œá´˜",   callback_data="adm_github",     style="primary"),
        Btn(f"{G['back']}  Aá´…á´ÉªÉ´",  callback_data="menu_admin",     style="primary"),
    )
...
def render_adm_gh_repos(call: types.CallbackQuery, force: bool = False) -> None:
    ...
    def _bg() -> None:
        ...
        rows = "\n".join(
            ...
            f"{'ðŸ”’' if r.get('private') else 'ðŸŒ'} "
            f"â­{r.get('stargazers_count',0)} "
            f"<i>{esc((r.get('description') or '')[:40])}</i>"
            for r in repos[:20]
        )
        cap = (
            f"<b>ðŸ™ {sc('Your GitHub Repos')} ({len(repos)})</b>\n"
            f"{G['div_eq']}\n{rows}\n{G['div']}\n"
...
def render_adm_gh_files(call: types.CallbackQuery, repo: str, path: str = "") -> None:
    ...
    ack(call, f"Loading {repo}/{path or 'root'}â€¦")
    def _bg() -> None:
        ...
        rows = "\n".join(
            f"{'ðŸ“' if it.get('type')=='dir' else 'ðŸ“„'} {esc(it.get('name','?'))} "
            + (f"<i>({fmt_bytes(it.get('size',0))})</i>" if it.get('type') != 'dir' else "")
            for it in items[:25]
        )
        cap = (
            f"<b>ðŸ“‚ {esc(breadcrumb[:50])}</b>\n"
            f"{G['div_eq']}\n{rows}\n"
            f"{G['div']}\n{len(items)} items{FOOTER}"
        )
        kb = types.InlineKeyboardMarkup(row_width=2)
        if path:
            kb.add(Btn("â¬†ï¸  Uá´˜",  callback_data="adm_gh_up", style="primary"))
        for i, it in enumerate(items[:12]):
            icon = "ðŸ“" if it.get("type") == "dir" else _file_icon(it.get("name",""))
            kb.add(Btn(f"{icon} {esc(it.get('name','?'))[:28]}", callback_data=f"adm_ghfile_{i}", style="primary"))
        kb.add(Btn(f"{G['back']}  Rá´‡á´˜á´êœ±", callback_data="adm_gh_repos", style="primary"))
        try:
            bot.send_message(call.from_user.id, cap, reply_markup=kb, parse_mode="HTML")
        except Exception:
            pass
...
def _file_icon(filename: str) -> str:
    ...
    return {"py":"ðŸ",".js":"ðŸ“œ",".json":"ðŸ“‹",".env":"ðŸ”",".txt":"ðŸ“",
            ".md":"ðŸ“",".zip":"ðŸ“¦",".sh":"âš™ï¸",".yaml":"ðŸ“‹",".yml":"ðŸ“‹",
            ".toml":"ðŸ“‹",".cfg":"âš™ï¸",".ini":"âš™ï¸",".html":"ðŸŒ",".css":"ðŸŽ¨"}.get(ext, "ðŸ“„")
...
def render_adm_gh_file_view(call: types.CallbackQuery, repo: str, path: str) -> None:
    ...
    ack(call, f"Loading {Path(path).name}â€¦")
    USER_STATES[call.from_user.id] = USER_STATES.get(call.from_user.id, {})
    ...
    def _bg() -> None:
        ...
        try:
            ...
            preview = "(binary file â€” cannot preview)"
        ...
        kb = types.InlineKeyboardMarkup(row_width=2)
        if runnable:
            kb.add(Btn("â–¶ï¸  Rá´œÉ´ Aêœ± Bá´á´›",  callback_data="adm_gh_run_file",  style="success"))
        kb.add(
            Btn("ðŸ“¥  Dá´á´¡É´ÊŸá´á´€á´…",        callback_data="adm_gh_dl_file",   style="primary"),
            Btn("ðŸ“  Bá´€á´„á´‹ á´›á´ Fá´ÊŸá´…á´‡Ê€",  callback_data="adm_gh_browse_repo",style="primary"),
        )
        kb.add(Btn(f"{G['back']}  GÊœ BÊ€á´á´¡êœ±á´‡Ê€", callback_data="adm_gh_browser", style="primary"))
        try:
            bot.send_message(call.from_user.id, cap, reply_markup=kb, parse_mode="HTML")
...
def action_adm_gh_run_file(call: types.CallbackQuery, repo: str, path: str) -> None:
    ...
    ack(call, f"Downloading and deploying {Path(path).name}â€¦")
    def _bg() -> None:
        try:
            branch = GH.get("branch", "main")
            ep = f"/repos/{repo}/contents/{path}?ref={branch}"
            ...
            if result.get("ok"):
                msg = (f"âœ… <b>{esc(bot_name)}</b> {sc('deployed and started from GitHub!')}\n"
                       f"{bullet('Bot ID', f'<code>{bid}</code>')}\n"
                       f"{bullet('Source', f'{esc(repo)}/{esc(path)}')} ")
            else:
                msg = (f"âš ï¸ <b>{esc(bot_name)}</b> {sc('uploaded but failed to start')}.\n"
        ...
        try:
            ...
            with tmp.open("rb") as fh:
                bot.send_document(call.from_user.id, fh,
                                  caption=f"ðŸ“¥ {esc(fname)} ({fmt_bytes(len(raw))})\n"
...
def render_adm_pay_config(call: types.CallbackQuery) -> None:
    ...
    currency_sym = get_setting("currency_symbol", "à§³")
    ...
    notif_chan = get_setting("payment_notif_channel", "") or "â€”"
    cap = (
        f"<b>ðŸ’³ {sc('Payment Configuration')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Auto-Approve',   'âœ… ON' if auto_approve else 'âŒ OFF')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ…' if auto_approve else 'âŒ'}  Aá´œá´›á´-Aá´˜á´˜Ê€",
            callback_data="adm_pay_auto_approve",
            style="success" if auto_approve else "danger"),
        Btn("ðŸ’°  Pá´€Ê Má´‡á´›Êœá´á´…êœ±",   callback_data="adm_pay_methods",      style="primary"),
    )
    kb.add(
        Btn("ðŸ“Š  Aá´á´á´œÉ´á´› LÉªá´Éªá´›êœ±",  callback_data="adm_pay_limits",       style="primary"),
        Btn("ðŸ’±  Cá´œÊ€Ê€á´‡É´á´„Ê",        callback_data="adm_pay_currency",     style="primary"),
    )
    kb.add(
        Btn("ðŸ§¾  Rá´‡á´„á´‡Éªá´˜á´› Tá´‡á´á´˜ÊŸ",  callback_data="adm_pay_receipt_tmpl", style="primary"),
        Btn("ðŸ””  Ná´á´›Éªêœ° Sá´‡á´›á´›ÉªÉ´É¢êœ±",  callback_data="adm_pay_notif",        style="primary"),
    )
    kb.add(
        Btn("ðŸ·ï¸  Sá´‡á´› Tá´€x %",       callback_data="adm_bc_set_payment_tax_pct",  style="primary"),
        Btn("ðŸ“‹  Pá´€Ê HÉªêœ±á´›á´Ê€Ê",    callback_data="adm_payments",         style="primary"),
    )
    kb.add(
        Btn("âœ…  Aá´˜á´˜Ê€á´á´ á´‡ Pá´€Ê",     callback_data="adm_approve",          style="success"),
        Btn("ðŸ“¤  Exá´˜á´Ê€á´› CSV",       callback_data="adm_user_export_csv",  style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_pay_methods(call: types.CallbackQuery) -> None:
    ...
    for key, m in PAYMENT_METHODS.items():
        ...
        rows.append(f"{'âœ…' if enabled else 'âŒ'} <b>{esc(m['name'])}</b> â€” "
                    f"<code>{esc(m['number'])}</code> ({esc(m['type'])})")
    cap = (
        f"<b>ðŸ’° {sc('Payment Methods')}</b>\n"
        f"{G['div_eq']}\n"
        + "\n".join(rows)
        + f"\n{G['div']}{FOOTER}"
    )
    ...
    for key, m in PAYMENT_METHODS.items():
        enabled = bool(get_setting(f"pm_enabled_{key}", True))
        kb.add(
            Btn(f"{'âœ…' if enabled else 'âŒ'} {esc(m['name'])}",
                callback_data=f"adm_pay_edit_{key}", style="primary"),
        )
    kb.add(Btn(f"{G['back']}  Pá´€Ê Cá´É´êœ°ÉªÉ¢", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_pay_method_edit(call: types.CallbackQuery, key: str) -> None:
    ...
    cap = (
        f"<b>ðŸ’° {sc('Edit')} {esc(m['name'])}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Status',  'âœ… Enabled' if enabled else 'âŒ Disabled')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âŒ Disable' if enabled else 'âœ… Enable'}",
            callback_data=f"adm_pay_method_toggle_{key}",
            style="danger" if enabled else "success"),
        Btn("âœï¸  CÊœá´€É´É¢á´‡ Ná´œá´Ê™á´‡Ê€",
            callback_data=f"adm_pay_method_setnumber_{key}", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Pá´€Ê Má´‡á´›Êœá´á´…êœ±", callback_data="adm_pay_methods", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_pay_limits(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ“Š {sc('Payment Amount Limits')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Min Payment',      f'{min_amt}à§³')}\n"
        f"{bullet('Max Payment',      f'{max_amt}à§³')}\n"
        f"{bullet('Discount >= à§³',   disc_threshold)}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“‰  Sá´‡á´› MÉªÉ´",         callback_data="adm_bc_set_min_payment_amount",  style="primary"),
        Btn("ðŸ“ˆ  Sá´‡á´› Má´€x",         callback_data="adm_bc_set_max_payment_amount",  style="primary"),
    )
    kb.add(
        Btn("ðŸŽ¯  DÉªêœ±á´„ TÊœÊ€á´‡êœ±Êœá´ÊŸá´…", callback_data="adm_bc_set_discount_threshold",  style="primary"),
        Btn("ðŸ’¸  DÉªêœ±á´„ %",           callback_data="adm_bc_set_discount_pct",        style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Pá´€Ê Cá´É´êœ°ÉªÉ¢", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_pay_currency(call: types.CallbackQuery) -> None:
    ...
    sym = get_setting("currency_symbol",  "à§³")
    cap = (
        f"<b>ðŸ’± {sc('Currency Settings')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Currency Code', cur)}\n"
        f"{bullet('Symbol',        sym)}\n"
        f"{G['div']}\n"
        f"{sc('Examples')}: BDT/à§³, USD/$, EUR/â‚¬, INR/â‚¹, PKR/â‚¨{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ”¤  Sá´‡á´› Cá´á´…á´‡",     callback_data="adm_bc_set_payment_currency", style="primary"),
        Btn("ðŸ’²  Sá´‡á´› SÊá´Ê™á´ÊŸ",   callback_data="adm_bc_set_currency_symbol",  style="primary"),
    )
    for code, sym_str in [("BDT","à§³"),("USD","$"),("EUR","â‚¬"),("INR","â‚¹"),("PKR","â‚¨")]:
...
def render_adm_pay_receipt_tmpl(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ§¾ {sc('Payment Receipt Template')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("âœï¸  Eá´…Éªá´›",      callback_data="adm_tmpl_edit_payment_received", style="primary"),
        Btn("ðŸ”„  Rá´‡êœ±á´‡á´›",     callback_data="adm_tmpl_reset_payment_received", style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Pá´€Ê Cá´É´êœ°ÉªÉ¢", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_pay_notif_settings(call: types.CallbackQuery) -> None:
    chan = get_setting("payment_notif_channel", "") or "â€”"
    ...
    cap = (
        f"<b>ðŸ”” {sc('Payment Notifications')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Channel',    esc(chan))}\n"
        f"{bullet('New payment', 'âœ…' if on_new else 'âŒ')}\n"
        f"{bullet('Approved',   'âœ…' if on_appr else 'âŒ')}\n"
        f"{bullet('Rejected',   'âœ…' if on_rej else 'âŒ')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("ðŸ“£  Sá´‡á´› CÊœá´€É´É´á´‡ÊŸ", callback_data="adm_bc_set_payment_notif_channel", style="primary"))
    kb.add(
        Btn(f"{'âœ…' if on_new else 'âŒ'}  Ná´‡á´¡ Pá´€Ê",  callback_data="adm_bc_toggle_notif_on_new_payment",  style="primary"),
        Btn(f"{'âœ…' if on_appr else 'âŒ'}  Aá´˜á´˜Ê€á´á´ á´‡á´…",callback_data="adm_bc_toggle_notif_on_approved",    style="primary"),
    )
    kb.add(Btn(f"{'âœ…' if on_rej else 'âŒ'}  Rá´‡á´Šá´‡á´„á´›á´‡á´…", callback_data="adm_bc_toggle_notif_on_rejected", style="primary"))
    kb.add(Btn(f"{G['back']}  Pá´€Ê Cá´É´êœ°ÉªÉ¢", callback_data="adm_pay_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bot_cfg(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("â±ï¸  TÉªá´á´‡á´á´œá´›êœ±",       callback_data="adm_bc_timeouts",  style="primary"),
        Btn("ðŸ“Š  LÉªá´Éªá´›êœ±",           callback_data="adm_bc_limits",    style="primary"),
    )
    kb.add(
        Btn("ðŸ“¦  Uá´˜ÊŸá´á´€á´… Rá´œÊŸá´‡êœ±",    callback_data="adm_bc_upload",    style="primary"),
        Btn("ðŸ”  EÉ´á´  Sá´›Ê€Éªá´˜",        callback_data="adm_bc_env",       style="danger"),
    )
    kb.add(
        Btn("ðŸ”„  Rá´‡êœ±á´›á´€Ê€á´› Pá´ÊŸÉªá´„Ê",  callback_data="adm_bc_policy",    style="primary"),
        Btn("ðŸ§±  Sá´€É´á´…Ê™á´x",           callback_data="adm_bc_sandbox",   style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_bc_timeouts(call: types.CallbackQuery) -> None:
    ...
    for k, label in [
        ...
        kb.add(Btn(f"âœï¸  {label}", callback_data=f"adm_bc_set_{k}", style="primary"))
    kb.add(Btn(f"{G['back']}  Bá´á´› Cá´É´êœ°ÉªÉ¢", callback_data="adm_bot_cfg", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_config", PHOTOS["admin"]), cap, kb, call=call)


def render_adm_bc_limits(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ“Š {sc('Resource Limits')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_bc_upload(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("âœï¸  Max Upload MB",    callback_data="adm_bc_set_max_upload_mb",       style="primary"),
        Btn("âœï¸  Allowed Ext",      callback_data="adm_bc_set_allowed_extensions",  style="primary"),
    )
    kb.add(
        Btn("âœï¸  Zip Max Files",    callback_data="adm_bc_set_zip_max_files",       style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Bá´á´› Cá´É´êœ°ÉªÉ¢", callback_data="adm_bot_cfg", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bc_env(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ” {sc('Environment Variable Control')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Strip Secrets', 'âœ… ON' if strip else 'âŒ OFF')}\n"
        f"{G['div']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ…' if strip else 'âŒ'}  Sá´›Ê€Éªá´˜ Sá´‡á´„Ê€á´‡á´›êœ±",
            callback_data="adm_bc_toggle_env_strip_secrets",
            style="success" if strip else "danger"),
        Btn("âž•  Aá´…á´… Sá´‡á´„Ê€á´‡á´› Ná´€á´á´‡",  callback_data="adm_bc_set_add_secret_name",   style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Bá´á´› Cá´É´êœ°ÉªÉ¢", callback_data="adm_bot_cfg", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bc_sandbox(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ§± {sc('Sandbox Settings')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('File Wipe',   'âœ… ON' if wipe else 'âŒ OFF')}\n"
        f"{bullet('Wipe Delay',  f'{delay}s after start')}\n"
        f"{bullet('Network',     'âœ… Allowed' if net else 'âŒ Blocked')}\n"
        f"{G['div']}\n"
        f"<i>{sc('File Wipe removes source .py/.js files after bot starts so child bots cannot read their own code.')}</i>{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ…' if wipe else 'âŒ'}  FÉªÊŸá´‡ WÉªá´˜á´‡",
            callback_data="adm_ff_toggle_sandbox_wipe",
            style="success" if wipe else "danger"),
        Btn("âœï¸  WÉªá´˜á´‡ Dá´‡ÊŸá´€Ê",     callback_data="adm_bc_set_sandbox_wipe_delay", style="primary"),
    )
    kb.add(
        Btn(f"{'âœ…' if net else 'âŒ'}  Ná´‡á´›á´¡á´Ê€á´‹",
...
def render_adm_bc_policy(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ”„ {sc('Restart & Policy')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Auto-Restart Crashed', 'âœ… ON' if auto_r else 'âŒ OFF')}\n"
        f"{bullet('Max Restarts/hour',    max_r)}\n"
        f"{bullet('Restart Delay',        str(delay_r) + 's')}\n"
        f"{bullet('Auto-Downgrade Expiry','âœ… ON' if auto_dg else 'âŒ OFF')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ…' if auto_r else 'âŒ'}  Aá´œá´›á´-Rá´‡êœ±á´›á´€Ê€á´›",
            callback_data="adm_ff_toggle_auto_restart_bots",
            style="success" if auto_r else "danger"),
        Btn("âœï¸  Má´€x Rá´‡êœ±á´›á´€Ê€á´›êœ±",   callback_data="adm_bc_set_max_crash_restarts", style="primary"),
    )
    kb.add(
        Btn("âœï¸  Rá´‡êœ±á´›á´€Ê€á´› Dá´‡ÊŸá´€Ê",  callback_data="adm_bc_set_crash_restart_delay", style="primary"),
        Btn(f"{'âœ…' if auto_dg else 'âŒ'}  Aá´œá´›á´-DÉ¢",
...
def render_adm_appearance(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸŽ¨ {sc('Appearance & Branding')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Theme',     esc(theme))}\n"
        f"{bullet('Brand Tag', esc(brand))}\n"
        f"{bullet('Footer',    esc(footer or '(default)'))}\n"
        f"{bullet('Custom Welcome', 'âœ…' if welcome else 'âŒ (default)')}\n"
        f"{bullet('Custom Rules',   'âœ…' if rules else 'âŒ (default)')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸŽ­  TÊœá´‡á´á´‡êœ±",            callback_data="adm_app_theme",      style="primary"),
        Btn("ðŸ·ï¸  BÊ€á´€É´á´… Tá´€É¢",         callback_data="adm_set_brand",      style="primary"),
    )
    kb.add(
        Btn("ðŸ“  Fá´á´á´›á´‡Ê€ Tá´‡xá´›",       callback_data="adm_set_footer_text",    style="primary"),
        Btn("ðŸ‘‹  Wá´‡ÊŸá´„á´á´á´‡ Mêœ±É¢",      callback_data="adm_set_welcome_text",   style="primary"),
    )
    kb.add(
        Btn("ðŸ“œ  Rá´œÊŸá´‡êœ± Tá´‡xá´›",        callback_data="adm_set_rules_text",     style="primary"),
        Btn("ðŸ˜€  Cá´œêœ±á´›á´á´ Eá´á´á´ŠÉªêœ±",     callback_data="adm_app_emojis",         style="primary"),
    )
    kb.add(
        Btn("ðŸ–¼ï¸  Má´‡É´á´œ PÊœá´á´›á´êœ±",      callback_data="adm_photos",             style="primary"),
        Btn("ðŸ“£  AÉ´É´ CÊœá´€É´É´á´‡ÊŸ",       callback_data="adm_set_announce",       style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_app_theme(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸŽ­ {sc('UI Themes')}</b>\n"
        f"{G['div_eq']}\n"
        f"{sc('Current')}: <b>{esc(cur)}</b>\n"
        f"{G['div']}\n"
        + "\n".join(
            f"{'âœ…' if k == cur else '  '} <b>{v['name']}</b> â€” "
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    for k, v in _APPEARANCE_THEMES.items():
        kb.add(Btn(f"{'âœ…' if k == cur else '  '} {v['name']}",
                   callback_data=f"adm_app_theme_{k}", style="primary"))
    kb.add(Btn(f"{G['back']}  Aá´˜á´˜á´‡á´€Ê€á´€É´á´„á´‡", callback_data="adm_appearance", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("appearance", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_app_emojis(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    for k in sample_keys:
        kb.add(Btn(f"âœï¸ {k}: {custom_emojis.get(k, G.get(k,'?'))}",
                   callback_data=f"adm_app_emoji_set_{k}", style="primary"))
    kb.add(Btn("ðŸ”„  Rá´‡êœ±á´‡á´› AÊŸÊŸ", callback_data="adm_app_emoji_reset", style="danger"))
    kb.add(Btn(f"{G['back']}  Aá´˜á´˜á´‡á´€Ê€á´€É´á´„á´‡", callback_data="adm_appearance", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("appearance", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_app_banner(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ–¼ï¸ {sc('Banner / Photo Settings')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("ðŸ–¼ï¸  Má´‡É´á´œ PÊœá´á´›á´êœ±", callback_data="adm_photos",     style="primary"))
    kb.add(Btn("ðŸ”„  Rá´‡Ê™á´œÉªÊŸá´… Bá´€É´É´á´‡Ê€êœ±", callback_data="adm_rebuild_banners", style="danger"))
    kb.add(Btn(f"{G['back']}  Aá´˜á´˜á´‡á´€Ê€á´€É´á´„á´‡", callback_data="adm_appearance", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("appearance", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_coupon_plus(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸŽ« {sc('Advanced Coupon Manager')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("âž•  CÊ€á´‡á´€á´›á´‡ Cá´á´œá´˜á´É´",   callback_data="adm_coupons",          style="success"),
        Btn("ðŸ—‚ï¸  Bá´œÊŸá´‹ CÊ€á´‡á´€á´›á´‡",      callback_data="adm_coupon_bulk",      style="primary"),
    )
    kb.add(
        Btn("ðŸ“Š  AÉ´á´€ÊŸÊá´›Éªá´„êœ±",        callback_data="adm_coupon_analytics", style="primary"),
        Btn("â°  Exá´˜ÉªÊ€Ê MÉ¢Ê€",        callback_data="adm_coupon_expiry",    style="primary"),
    )
    kb.add(
        Btn("ðŸ—‘ï¸  CÊŸá´‡á´€Ê€ Exá´˜ÉªÊ€á´‡á´…",   callback_data="adm_coupon_clearexp",  style="danger"),
        Btn("ðŸ“‹  AÊŸÊŸ Cá´á´œá´˜á´É´êœ±",      callback_data="adm_coupons",          style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_coupon_analytics(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ“Š {sc('Coupon Analytics')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total Coupons',    len(coupons))}\n"
        f"{bullet('Total Savings Given', f'{total_savings:.0f}à§³')}\n"
        f"{G['div']}\n<b>{sc('By Plan')}:</b>\n{plan_rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS.get("coupon_plus", PHOTOS["coupon"]), cap,
              _adm_back("adm_coupon_plus"), call=call)
...
def render_adm_coupon_expiry(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>â° {sc('Coupon Expiry Manager')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(Btn("ðŸ—‘ï¸  CÊŸá´‡á´€Ê€ Exá´˜ÉªÊ€á´‡á´…", callback_data="adm_coupon_clearexp", style="danger"))
    kb.add(Btn(f"{G['back']}  Cá´á´œá´˜á´É´ MÉ¢Ê€", callback_data="adm_coupon_plus", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("coupon_plus", PHOTOS["coupon"]), cap, kb, call=call)
...
def render_adm_templates(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ“ {sc('Message Template Manager')}</b>\n"
        f"{G['div_eq']}\n"
        ...
        + "\n".join(
            ...
            f"{'âœ… custom' if get_setting(f'tmpl_{k}') else 'ðŸ“„ default'}"
    ...
    for k, v in _MESSAGE_TEMPLATES.items():
        ...
        kb.add(Btn(f"{'âœ…' if has_custom else 'ðŸ“„'} {v['label'][:25]}",
...
def render_adm_referral_sys(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ”— {sc('Referral System')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Status',        'âœ… Enabled' if enabled else 'âŒ Disabled')}\n"
        f"{bullet('Reward/Refer',  f'{reward}à§³ wallet credit')}\n"
        f"{bullet('Min Plan',      min_plan)}\n"
        f"{bullet('Total Referrals', total_refs)}\n"
        f"{bullet('Total Paid Out',  f'{total_paid}à§³')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ… Enabled' if enabled else 'âŒ Disabled'}",
            callback_data="adm_ref_toggle",
            style="success" if enabled else "danger"),
        Btn("ðŸ“Š  Rá´‡êœ° Sá´›á´€á´›êœ±",    callback_data="adm_ref_stats",       style="primary"),
    )
    kb.add(
        Btn("ðŸŽ  Rá´‡á´¡á´€Ê€á´… Cá´É´êœ°ÉªÉ¢",callback_data="adm_ref_rewards",     style="primary"),
        Btn("ðŸ†  Lá´‡á´€á´…á´‡Ê€Ê™á´á´€Ê€á´…",   callback_data="adm_ref_leaderboard", style="primary"),
    )
    kb.add(
        Btn("âœï¸  Sá´‡á´› Rá´‡á´¡á´€Ê€á´… à§³", callback_data="adm_ref_set_reward",   style="primary"),
        Btn("âœï¸  Sá´‡á´› MÉªÉ´ PÊŸá´€É´", callback_data="adm_ref_set_min_plan", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_ref_stats(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{i}. {esc(u.get('name','?')[:20])} â€” {len(u.get('referrals',[]))} refs "
        f"| earned {u.get('referral_earnings',0)}à§³"
        for i, (uid, u) in enumerate(top_refs, 1)
    ) or f"<i>{sc('No referrals yet')}</i>"
    cap = (
        f"<b>ðŸ“Š {sc('Referral Statistics')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Total Referrals', total_refs)}\n"
        f"{bullet('Today',           today_refs)}\n"
        f"{bullet('Total Paid',      f'{total_paid}à§³')}\n"
...
def render_adm_ref_rewards(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸŽ {sc('Referral Reward Config')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Base Reward',         f'{reward}à§³ per referral')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ’°  Sá´‡á´› Bá´€êœ±á´‡ Rá´‡á´¡á´€Ê€á´…",   callback_data="adm_ref_set_reward",      style="primary"),
        Btn("âœï¸  Sá´‡á´› Bá´É´á´œêœ± TÊœÊ€",     callback_data="adm_bc_set_referral_bonus_threshold", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Rá´‡êœ°á´‡Ê€Ê€á´€ÊŸ SÊêœ±", callback_data="adm_referral_sys", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("referral_adm", PHOTOS["referral"]), cap, kb, call=call)
...
def render_adm_ref_leaderboard(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{i}. <b>{esc(u.get('name','?')[:20])}</b> â€” "
        f"{len(u.get('referrals',[]))} {sc('refs')} | "
        f"{u.get('referral_earnings',0)}à§³ {sc('earned')}"
        for i, (uid, u) in enumerate(top, 1)
    ) or f"<i>{sc('No referrals yet')}</i>"
    cap = (
        f"<b>ðŸ† {sc('Referral Leaderboard')}</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS.get("referral_adm", PHOTOS["referral"]), cap,
              _adm_back("adm_referral_sys"), call=call)
...
def render_adm_janitor(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ§¹ {sc('Janitor â€” Auto-Cleanup Rules')}</b>\n"
        f"{G['div_eq']}\n"
        + "\n".join(
            f"{'âœ…' if get_setting(f'jan_{k}', False) else 'âŒ'} {v}"
    ...
    for k, v in flags.items():
        ...
        kb.add(Btn(f"{'âœ…' if on else 'âŒ'} {v[:28]}",
                   callback_data=f"adm_jan_toggle_{k}", style="primary"))
    kb.add(
        Btn("â–¶ï¸  Rá´œÉ´ Ná´á´¡",         callback_data="adm_jan_run_now",   style="success"),
        Btn("ðŸ“‹  Já´€É´ Rá´œÊŸá´‡êœ±",       callback_data="adm_jan_rules",     style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_jan_rules(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ“‹ {sc('Janitor Rule Details')}</b>\n"
        f"{G['div_eq']}\n"
...
def action_adm_jan_run(admin_uid: int) -> None:
    ...
    if get_setting("jan_clean_orphan_dirs", False):
        try:
            dirs, files = _do_clean_orphans()
            results.append(f"âœ… Orphan cleanup: {dirs} dirs, {files} files removed")
        except Exception as e:
            results.append(f"âŒ Orphan cleanup: {e}")
    # Expired coupons
    if get_setting("jan_clean_expired_coupons", False):
        try:
            ...
            removed = before - len(d["coupons"])
            db_save(d)
            results.append(f"âœ… Expired coupons: {removed} removed")
        except Exception as e:
            results.append(f"âŒ Coupon cleanup: {e}")
    # Audit trim
...
def render_adm_webhooks(call: types.CallbackQuery) -> None:
    ...
    last_err= wh_info.get("last_error_message", "â€”")
    cap = (
        f"<b>ðŸŒ {sc('Webhook Manager')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Mode',           mode)}\n"
        f"{bullet('Webhook URL',    esc(wh_url[:50]) if wh_url else 'â€”')}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ”—  Sá´‡á´› Wá´‡Ê™Êœá´á´á´‹",    callback_data="adm_wh_set",   style="primary"),
        Btn("âŒ  CÊŸá´‡á´€Ê€ (Pá´ÊŸÊŸÉªÉ´É¢)", callback_data="adm_wh_clear", style="danger"),
    )
    kb.add(
        Btn("ðŸ§ª  Tá´‡êœ±á´› Wá´‡Ê™Êœá´á´á´‹",   callback_data="adm_wh_test",  style="primary"),
        Btn("â„¹ï¸  Wá´‡Ê™Êœá´á´á´‹ IÉ´êœ°á´",   callback_data="adm_wh_info",  style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    ...
    def _bg() -> None:
        try:
            ...
            with _ur.urlopen(req, timeout=10) as resp:
                status = resp.status
            bot.send_message(call.from_user.id,
                             f"{G['ok']} {sc('Webhook test')}: HTTP {status} âœ…")
        except Exception as e:
            try:
                bot.send_message(call.from_user.id,
...
def render_adm_wh_info(call: types.CallbackQuery) -> None:
    try:
        ...
        cap = (
            f"<b>â„¹ï¸ {sc('Webhook Info')}</b>\n"
            f"{G['div_eq']}\n"
            f"{bullet('URL',             esc(str(wi.url or 'â€”')[:60]))}\n"
            ...
            f"{bullet('Last Error',      esc(str(wi.last_error_message or 'â€”')[:60]))}\n"
            f"{bullet('Last Error Time', fmt_ts(wi.last_error_date))}\n"
            f"{bullet('IP Address',      wi.ip_address or 'â€”')}\n"
            f"{G['div']}{FOOTER}"
        )
    except Exception as e:
...
def render_adm_feature_flags(call: types.CallbackQuery) -> None:
    ...
    for k, default in _FEATURE_FLAG_DEFAULTS.items():
        val = _ff_get(k)
        rows.append(f"{'âœ…' if val else 'âŒ'} <code>{k}</code>")
    cap = (
        f"<b>ðŸŽ¯ {sc('Feature Flags')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    for k in list(_FEATURE_FLAG_DEFAULTS.keys()):
        ...
        kb.add(Btn(f"{'âœ…' if val else 'âŒ'} {label}",
                   callback_data=f"adm_ff_toggle_{k}", style="primary"))
    kb.add(Btn("ðŸ”„  Rá´‡êœ±á´‡á´› AÊŸÊŸ FÊŸá´€É¢êœ±", callback_data="adm_ff_reset_all", style="danger"))
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_rate_config(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>â±ï¸ {sc('Rate Limit Configuration')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Global Rate Limiting', 'âœ… ON' if global_rl else 'âŒ OFF')}\n"
        f"{G['div']}\n"
        + "\n".join(
            f"<b>{PLAN_LIMITS.get(plan,{}).get('name', plan)}</b>: "
            f"â†‘{get_setting(f'rl_{plan}_uploads_per_day', d['uploads_per_day'])}/day "
            f"â–¶{get_setting(f'rl_{plan}_starts_per_hour', d['starts_per_hour'])}/hr "
            f"ðŸ’¬{get_setting(f'rl_{plan}_msgs_per_min', d['msgs_per_min'])}/min"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn(f"{'âœ… ON' if global_rl else 'âŒ OFF'}  GÊŸá´Ê™á´€ÊŸ RL",
    ...
    for plan in _RATE_LIMIT_DEFAULTS:
        ...
        kb.add(Btn(f"âœï¸  {name}", callback_data=f"adm_rate_plan_{plan}", style="primary"))
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_rate_plan(call: types.CallbackQuery, plan: str) -> None:
    ...
    cap = (
        f"<b>â±ï¸ {sc('Rate Limits for')} {esc(name)}</b>\n"
        f"{G['div_eq']}\n"
    ...
    for metric in ("uploads_per_day", "starts_per_hour", "msgs_per_min"):
        kb.add(Btn(f"âœï¸  {metric.replace('_',' ').title()}",
                   callback_data=f"adm_rate_set_{plan}_{metric}", style="primary"))
    kb.add(Btn(f"{G['back']}  Rá´€á´›á´‡ Cá´É´êœ°ÉªÉ¢", callback_data="adm_rate_config", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("rate_limits", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_live_monitor(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ”„  Rá´‡êœ°Ê€á´‡êœ±Êœ",         callback_data="adm_monitor_refresh",  style="success"),
        Btn("ðŸ¤–  Bá´á´› Dá´‡á´›á´€ÉªÊŸêœ±",     callback_data="adm_monitor_bots",     style="primary"),
    )
    kb.add(
        Btn("ðŸ–¥ï¸  SÊêœ±á´›á´‡á´",           callback_data="adm_monitor_system",   style="primary"),
        Btn("ðŸ’¥  CÊ€á´€êœ±Êœá´‡á´…",          callback_data="adm_crashed_bots",     style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    ...
    for bid, info in list(RUNNING.items())[:20]:
        ...
        if psutil and is_running:
            try:
                ...
                pass
        status = "â–¶ running" if is_running else f"â¹ exit={rc}"
        rows.append(
            f"{G['bullet']} <b>{esc(name)}</b> <code>{bid[:8]}</code>\n"
            f"   {status} | PID {pid} | {fmt_bytes(rss)} | CPU {cpu:.1f}%"
        )
    cap = (
        f"<b>ðŸ¤– {sc('Bot Monitor')} ({len(RUNNING)} total)</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_monitor_system(call: types.CallbackQuery) -> None:
    ...
    def bar(pct: float) -> str:
        filled = int(pct / 10)
        return "â–ˆ" * filled + "â–‘" * (10 - filled) + f" {pct:.1f}%"
    cap = (
        f"<b>ðŸ–¥ï¸ {sc('System Monitor')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_rev_goals(call: types.CallbackQuery) -> None:
    ...
    def progress_bar(cur: float, goal: float) -> str:
        if not goal:
            return "â€” (no goal set)"
        pct = min(100, cur * 100 / goal)
        filled = int(pct / 5)
        return "â–ˆ" * filled + "â–‘" * (20 - filled) + f" {pct:.1f}%"
    cap = (
        f"<b>ðŸ’Ž {sc('Revenue Goals')}</b>\n"
        f"{G['div_eq']}\n"
        f"<b>{sc('This Month')} ({month_start})</b>\n"
        f"  {sc('Earned')}: <b>{rev_month}à§³</b> / {goal_month or '?'}à§³\n"
        f"  {progress_bar(rev_month, goal_month)}\n"
        f"{G['div']}\n"
        f"<b>{sc('This Year')} ({year_start})</b>\n"
        f"  {sc('Earned')}: <b>{rev_year}à§³</b> / {goal_year or '?'}à§³\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸŽ¯  Sá´‡á´› Má´É´á´›ÊœÊŸÊ Gá´á´€ÊŸ", callback_data="adm_goal_set_monthly", style="primary"),
        Btn("ðŸŽ¯  Sá´‡á´› Yá´‡á´€Ê€ÊŸÊ Gá´á´€ÊŸ",  callback_data="adm_goal_set_yearly",  style="primary"),
    )
    kb.add(
        Btn("ðŸ“ˆ  HÉªêœ±á´›á´Ê€Ê",           callback_data="adm_goal_history",     style="primary"),
        Btn("ðŸ“Š  Rá´‡á´ á´‡É´á´œá´‡ Rá´‡á´˜á´Ê€á´›",   callback_data="adm_revenue_report",   style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_scheduler(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div']}\n"
        + ("\n".join(
            f"{G['bullet']} {'âœ…' if t.get('enabled',True) else 'â¸ï¸'} "
            f"<b>{esc(t.get('type','?'))}</b> {t.get('time','?')} â€” "
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("âž•  Aá´…á´… Tá´€êœ±á´‹",      callback_data="adm_sched_add",  style="success"),
        Btn("ðŸ“‹  AÊŸÊŸ Tá´€êœ±á´‹êœ±",     callback_data="adm_sched_list", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_sched_list(call: types.CallbackQuery) -> None:
    tasks = get_setting("scheduled_tasks", []) or []
    cap = (
        f"<b>ðŸ“‹ {sc('Scheduled Tasks')}</b>\n"
        f"{G['div_eq']}\n"
        + ("\n".join(
            f"{G['bullet']} <code>{t.get('id','?')[:8]}</code> "
            f"{'âœ…' if t.get('enabled',True) else 'â¸ï¸'} "
    ...
    for t in tasks[:8]:
        ...
        kb.add(
            Btn(f"{'â¸ï¸' if en else 'â–¶ï¸'} {tid[:8]}",
                callback_data=f"adm_sched_toggle_{tid}", style="primary"),
            Btn(f"ðŸ—‘ï¸ {tid[:8]}",
                callback_data=f"adm_sched_del_{tid}",    style="danger"),
        )
    kb.add(Btn("âž•  Aá´…á´… Tá´€êœ±á´‹",     callback_data="adm_sched_add",   style="success"))
    kb.add(Btn(f"{G['back']}  Sá´„Êœá´‡á´…á´œÊŸá´‡Ê€", callback_data="adm_scheduler", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("scheduler", PHOTOS["admin"]), cap, kb, call=call)


def _sched_check_and_run() -> None:
    """Background thread â€” runs every 60s, fires scheduled tasks."""
    while True:
        try:
            time.sleep(60)
            tasks = get_setting("scheduled_tasks", []) or []
...
def render_adm_import_export(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“¤  Exá´˜á´Ê€á´› Cá´É´êœ°ÉªÉ¢",  callback_data="adm_export_full_cfg", style="success"),
        Btn("ðŸ“¥  Iá´á´˜á´Ê€á´› Cá´É´êœ°ÉªÉ¢",  callback_data="adm_import_cfg",      style="primary"),
    )
    kb.add(
        Btn("ðŸ‘¥  Exá´˜á´Ê€á´› Uêœ±á´‡Ê€êœ± CSV",callback_data="adm_user_export_csv", style="primary"),
        Btn("ðŸ—„ï¸  Fá´Ê€á´„á´‡ Bá´€á´„á´‹á´œá´˜",   callback_data="adm_force_backup",    style="primary"),
    )
    kb.add(
        Btn("â™»ï¸  Fá´€á´„á´›á´Ê€Ê Rá´‡êœ±á´‡á´›",  callback_data="adm_import_reset",    style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def action_adm_export_full_cfg(call: types.CallbackQuery) -> None:
    if not is_owner(call.from_user.id):
        ack(call, "Owner only"); return
    ack(call, "Preparing config exportâ€¦")
    def _bg() -> None:
        try:
            import json as _j
            settings = {}
            ...
            with tmp.open("rb") as fh:
                bot.send_document(call.from_user.id, fh,
                                  caption=f"ðŸ“¥ {sc('Config Export')} â€” {ts_iso()[:10]}",
...
def render_adm_admin_2fa(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸ” {sc('Admin 2FA')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Status',  'âœ… Enabled' if enabled else 'âŒ Disabled')}\n"
        f"{bullet('Secret',  'âœ… Set' if secret else 'âŒ Not configured')}\n"
        f"{G['div']}\n"
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    if not secret:
        kb.add(Btn("ðŸ”‘  Sá´‡á´›á´œá´˜ 2FA", callback_data="adm_2fa_setup", style="success"))
    else:
        kb.add(
            Btn(f"{'âœ… ON' if enabled else 'âŒ OFF'}  Tá´É¢É¢ÊŸá´‡",
                callback_data="adm_bc_toggle_admin_2fa_enabled",
                style="success" if enabled else "danger"),
            Btn("ðŸ—‘ï¸  DÉªêœ±á´€Ê™ÊŸá´‡+Rá´‡êœ±á´‡á´›", callback_data="adm_2fa_disable", style="danger"),
        )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_leaderboard(call: types.CallbackQuery) -> None:
    cap = (
        f"<b>ðŸ† {sc('Leaderboard')}</b>\n"
        f"{G['div_eq']}\n"
        f"{sc('View top users by different metrics.')}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ’°  Tá´á´˜ Sá´˜á´‡É´á´…á´‡Ê€êœ±",   callback_data="adm_lb_spenders",  style="success"),
        Btn("ðŸ¤–  Má´êœ±á´› Bá´á´›êœ±",      callback_data="adm_lb_bots",       style="primary"),
    )
    kb.add(
        Btn("ðŸ”—  Tá´á´˜ Rá´‡êœ°á´‡Ê€Ê€á´‡Ê€êœ±",  callback_data="adm_lb_referrals",  style="primary"),
        Btn("âš¡  Má´êœ±á´› Aá´„á´›Éªá´ á´‡",    callback_data="adm_lb_active",     style="primary"),
    )
    kb.add(
        Btn("â±ï¸  Lá´É´É¢á´‡êœ±á´› Uá´˜á´›Éªá´á´‡", callback_data="adm_lb_uptime",     style="primary"),
        Btn("ðŸ†  AÊŸÊŸ LÊ™êœ±",         callback_data="adm_top_users",     style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_lb_spenders(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"<code>{uid}</code> â€” <b>{amt:.0f}à§³</b>"
        for i, (uid, amt) in enumerate(top, 1)
    ) or f"<i>{sc('No data')}</i>"
    cap = f"<b>ðŸ’° {sc('Top Spenders')}</b>\n{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
...
def render_adm_lb_bots(call: types.CallbackQuery) -> None:
    ...
    users_db = db_load()["users"]
    rows = "\n".join(
        f"{i}. <b>{esc(users_db.get(uid,{}).get('name','?')[:20])}</b> â€” {n} bots"
        for i, (uid, n) in enumerate(top, 1)
    ) or f"<i>{sc('No data')}</i>"
    cap = f"<b>ðŸ¤– {sc('Most Bots')}</b>\n{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    show_menu(call.message.chat.id, PHOTOS.get("leaderboard", PHOTOS["stats"]), cap,
              _adm_back("adm_leaderboard"), call=call)
...
def render_adm_lb_referrals(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        f"{i}. <b>{esc(u.get('name','?')[:20])}</b> â€” "
        f"{len(u.get('referrals',[]))} refs | {u.get('referral_earnings',0)}à§³"
        for i, (uid, u) in enumerate(top, 1) if u.get("referrals")
    ) or f"<i>{sc('No referrals yet')}</i>"
    cap = f"<b>ðŸ”— {sc('Top Referrers')}</b>\n{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    show_menu(call.message.chat.id, PHOTOS.get("leaderboard", PHOTOS["stats"]), cap,
              _adm_back("adm_leaderboard"), call=call)
...
def render_adm_lb_active(call: types.CallbackQuery) -> None:
    ...
    cap = f"<b>âš¡ {sc('Most Recently Active')}</b>\n{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    show_menu(call.message.chat.id, PHOTOS.get("leaderboard", PHOTOS["stats"]), cap,
              _adm_back("adm_leaderboard"), call=call)
...
def render_adm_lb_uptime(call: types.CallbackQuery) -> None:
    ...
    for i, (bid, info) in enumerate(running[:15], 1):
        ...
        b       = find_bot(bid)
        name    = (b.get("name","?") if b else bid)[:20]
        rows.append(f"{i}. <b>{esc(name)}</b> â€” {fmt_dur(uptime * 1000)} uptime")
    cap = (
        f"<b>â±ï¸ {sc('Longest Uptime Bots')}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_languages(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>ðŸŒ {sc('Language Settings')}</b>\n"
        f"{G['div_eq']}\n"
    ...
    for code, name in _SUPPORTED_LANGUAGES.items():
        kb.add(Btn(f"{'âœ…' if code == cur else '  '} {name}",
...
def render_adm_bot_controls_panel(call: types.CallbackQuery) -> None:
    ...
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“‹  LÉªêœ±á´› AÊŸÊŸ Bá´á´›êœ±",   callback_data="adm_bc_list_all",  style="primary"),
        Btn("ðŸ”  Sá´‡á´€Ê€á´„Êœ Bá´á´›",       callback_data="adm_bot_search",   style="primary"),
    )
    kb.add(
        Btn("ðŸ’¥  CÊ€á´€êœ±Êœá´‡á´… Bá´á´›êœ±",    callback_data="adm_crashed_bots", style="danger"),
    ...
    kb.add(
        Btn("ðŸ”´  KÉªÊŸÊŸ AÊŸÊŸ",         callback_data="adm_kill_all_now", style="danger"),
        Btn("ðŸ”„  Rá´‡êœ±á´›á´€Ê€á´› Sá´›á´á´˜á´˜á´‡á´…",  callback_data="adm_mass_restart_stopped", style="success"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_bc_list_all(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"uid={b.get('owner','?')} "
        f"{'â–¶' if bid in RUNNING and RUNNING[bid]['proc'].poll() is None else 'â¹'}"
        for bid, b in page_bots
    ) or f"<i>{sc('No bots')}</i>"
    cap = (
        f"<b>ðŸ“‹ {sc('All Bots')} ({len(bots)} total)</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    for bid, b in page_bots:
        is_running = bid in RUNNING and RUNNING[bid]["proc"].poll() is None
        icon = "â–¶" if is_running else "â¹"
        kb.add(Btn(f"{icon} {esc(b.get('name','?')[:22])}",
                   callback_data=f"adm_bcbot_{bid[:20]}", style="primary"))
    kb.add(Btn(f"{G['back']}  Bá´á´› Cá´É´á´›Ê€á´ÊŸêœ±", callback_data="adm_bot_controls", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_controls", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bc_single(call: types.CallbackQuery, bid: str) -> None:
    ...
    if psutil and is_running and pid:
        try:
            ...
            pass
    cap = (
        f"<b>ðŸ¤– {esc(b.get('name','?'))}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('ID',       f'<code>{bid}</code>')}\n"
        f"{bullet('Owner',    str(b.get('owner','?')))}\n"
        f"{bullet('Plan',     b.get('plan','free'))}\n"
        f"{bullet('Status',   'â–¶ Running' if is_running else 'â¹ Stopped')}\n"
        f"{bullet('PID',      pid or 'â€”')}\n"
        f"{bullet('RAM',      fmt_bytes(rss) if rss else 'â€”')}\n"
    ...
    if is_running:
        kb.add(
            Btn("â¹  Sá´›á´á´˜",       callback_data=f"adm_bc_stop_{bid[:20]}",    style="danger"),
            Btn("ðŸ”„  Rá´‡êœ±á´›á´€Ê€á´›",   callback_data=f"adm_bc_restart_{bid[:20]}", style="success"),
        )
    else:
        kb.add(Btn("â–¶ï¸  Sá´›á´€Ê€á´›",   callback_data=f"adm_bc_restart_{bid[:20]}", style="success"))
    kb.add(
        Btn("ðŸ“‹  Lá´É¢êœ±",           callback_data=f"adm_bc_logs_{bid[:20]}",    style="primary"),
        Btn("ðŸ”  EÉ´á´  Eá´…Éªá´›á´Ê€",    callback_data=f"adm_bc_env_{bid[:20]}",     style="primary"),
    )
    kb.add(
        Btn("ðŸ“Š  Rá´‡êœ±á´á´œÊ€á´„á´‡êœ±",     callback_data=f"adm_bc_res_{bid[:20]}",     style="primary"),
        Btn("ðŸ—‘ï¸  Dá´‡ÊŸá´‡á´›á´‡",        callback_data=f"adm_bc_del_{bid[:20]}",     style="danger"),
    )
    kb.add(Btn(f"{G['back']}  AÊŸÊŸ Bá´á´›êœ±", callback_data="adm_bc_list_all", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("bot_controls", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_bc_env_editor(call: types.CallbackQuery, bid: str) -> None:
    ...
    cap = (
        f"<b>ðŸ” {sc('Env Editor')}: {esc(b.get('name','?'))}</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}\n"
        ...
        for root, _, files in os.walk(bot_dir):
            for f in files:
                try:
                    ...
                    pass
    cap = (
        f"<b>ðŸ“Š {sc('Resource Usage')}: {esc(b.get('name','?'))}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Status',   'â–¶ Running' if is_running else 'â¹ Stopped')}\n"
...
def render_adm_bc_logs(call: types.CallbackQuery, bid: str) -> None:
    ...
    cap = (
        f"<b>ðŸ“‹ {sc('Logs')}: {esc(b.get('name','?'))}</b>\n"
        f"{G['div_eq']}\n"
...
def render_adm_subscriptions(call: types.CallbackQuery) -> None:
    ...
    auto_dg = bool(get_setting("auto_downgrade_expired", True))
    cap = (
        f"<b>ðŸ‘¤ {sc('Subscription Manager')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Paid Users',       len(paid_users))}\n"
        f"{bullet('Expiring in 7d',   len(expiring_7d))}\n"
        f"{bullet('Already Expired',  len(expired_sub))}\n"
        f"{bullet('Auto-Downgrade',   'âœ… ON' if auto_dg else 'âŒ OFF')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("â°  Exá´˜ÉªÊ€ÉªÉ´É¢ Sá´á´É´",    callback_data="adm_sub_expiring",      style="danger"),
        Btn("âŒ  Exá´˜ÉªÊ€á´‡á´…",           callback_data="adm_sub_expired",        style="primary"),
    )
    kb.add(
        Btn("ðŸ“¨  Rá´‡á´ÉªÉ´á´… AÊŸÊŸ",       callback_data="adm_sub_remind_all",     style="success"),
        Btn("âž•  Exá´›á´‡É´á´… Sá´œÊ™",       callback_data="adm_sub_extend_prompt",  style="primary"),
    )
    kb.add(
        Btn(f"{'âœ…' if auto_dg else 'âŒ'}  Aá´œá´›á´-DÉ¢",
            callback_data="adm_sub_auto_downgrade",
            style="success" if auto_dg else "danger"),
        Btn("âš¡  Rá´œÉ´ Dá´á´¡É´É¢Ê€á´€á´…á´‡",    callback_data="adm_sub_run_downgrade",  style="danger"),
    )
    kb.add(
        Btn("ðŸ“‹  Sá´œÊ™ HÉªêœ±á´›á´Ê€Ê",      callback_data="adm_sub_history",        style="primary"),
        Btn("ðŸ’°  Pá´€Êá´á´‡É´á´›êœ±",         callback_data="adm_payments",           style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_sub_expiring(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>â° {sc('Expiring in 7 Days')} ({len(expiring)})</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
...
def render_adm_sub_expired(call: types.CallbackQuery) -> None:
    ...
    cap = (
        f"<b>âŒ {sc('Expired Subscriptions')} ({len(expired)})</b>\n"
        f"{G['div_eq']}\n{rows}\n{G['div']}{FOOTER}"
    )
    show_menu(call.message.chat.id, PHOTOS.get("subscriptions", PHOTOS["admin"]), cap,
              _adm_back("adm_subscriptions"), call=call)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• END MEGA ADVANCED PANELS â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def on_photo(m: types.Message) -> None:
    ...
    if not require_verified(m.chat.id, uid):
        return
    st = USER_STATES.get(uid) or {}
    # â”€â”€ admin sent a banner replacement â”€â”€
...
def on_text(m: types.Message) -> None:
    ...
    try:
        ...
        if flow == "await_set_brand":
            ...
            if not new:
                bot.reply_to(m, f"{G['no']} {sc('empty â€” cancelled')}")
                USER_STATES.pop(uid, None); return
            global BRAND_TAG
            BRAND_TAG = new
        ...
        if flow == "await_set_owner":
            ...
            try:
                ...
                bot.reply_to(m, f"{G['no']} {sc('invalid id â€” send a positive integer')}")
                return
            global OWNER_ID
            OWNER_ID = new_owner
        ...
        # â”€â”€ New advanced admin flows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if (q.isdigit() and _uid == q) or \
                   ...
                   q in str(u.get("name", "")).lower():
                    ...
                    results.append(
                        ...
                        f"@{esc(u.get('username','â€”'))} "
                        f"plan={u.get('plan','free')} "
                        f"bots={bot_count} "
                        f"wallet={u.get('wallet',0)}à§³ "
                        f"{'ðŸš«banned' if u.get('banned') else ''}"
                    )
            USER_STATES.pop(uid, None)
            reply = ("\n".join(results[:10]) or f"<i>{sc('No users found for')}: {esc(q)}</i>")
            bot.reply_to(m, f"<b>ðŸ” {sc('Search Results')}</b>\n{G['div_eq']}\n{reply}",
                         parse_mode="HTML")
            return
        ...
        if flow == "await_adm_notify_user":
            ...
            try:
                bot.send_message(int(target_uid_str),
                                 f"<b>ðŸ“¨ {sc('Message from Admin')}</b>\n{G['div']}\n{esc(msg_text)}",
                                 parse_mode="HTML")
            ...
            for bid, b in bots.items():
                if q in bid.lower() or q in b.get("name", "").lower():
                    ...
                    results.append(
                        ...
                        f"uid={b.get('owner')} "
                        f"{'â–¶ running' if running else 'â¹ stopped'}"
                    )
            USER_STATES.pop(uid, None)
            reply = "\n".join(results[:10]) or f"<i>{sc('No bots found')}</i>"
            bot.reply_to(m, f"<b>ðŸ” {sc('Bot Search')}</b>\n{G['div_eq']}\n{reply}",
                         parse_mode="HTML")
            return
        ...
        if flow == "await_adm_notify_running":
            ...
            USER_STATES.pop(uid, None)
            if not msg_text:
                bot.reply_to(m, f"{G['no']} {sc('Empty message â€” cancelled')}."); return
            def _bg_nr() -> None:
                sent = fail = 0
                for t_uid in target_uids:
                    try:
                        bot.send_message(int(t_uid),
                                         f"<b>ðŸ“¢ {sc('Admin Message')}</b>\n{G['div']}\n{esc(msg_text)}",
                                         parse_mode="HTML")
                        sent += 1
                    except Exception:
                        fail += 1
            ...
            bot.reply_to(m, f"{G['ok']} {sc('Sending to')} {len(target_uids)} {sc('users')}â€¦")
            return
        ...
        # â”€â”€â”€ MEGA ADVANCED PANEL FLOWS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if flow == "await_adm_ref_reward":
            ...
            try:
                ...
                bot.reply_to(m, f"{G['ok']} {sc('Referral reward set to')} {amount}à§³")
            except ValueError:
        ...
        if flow == "await_adm_goal_set":
            ...
            try:
                ...
                bot.reply_to(m, f"{G['ok']} {goal_type.title()} {sc('goal set to')} {amount}à§³")
            except ValueError:
                bot.reply_to(m, f"{G['no']} {sc('Please send a valid integer.')}")
            return
        ...
        if flow == "await_adm_factory_reset":
            ...
            if text.strip() != "CONFIRM RESET":
                bot.reply_to(m, f"{G['no']} {sc('Cancelled â€” must send exactly')} "
            ...
            try:
                ...
                bot.reply_to(m, f"â™»ï¸ {sc('Factory reset done â€” all settings wiped. Bot restart recommended.')}")
            ...
            try:
                ...
                try:
                    bot.send_message(int(target_uid),
                                     f"ðŸŽ {sc('Your subscription has been extended by')} "
        ...
        if flow == "await_adm_sub_history":
            ...
            rows = "\n".join(
                ...
                f"<b>{p.get('plan','?')}</b> {p.get('amount','?')}à§³"
                for p in pays[:15]
            ) or f"<i>{sc('No payment history')}</i>"
            cap = (
                f"<b>ðŸ“‹ {sc('Sub History')}: {esc(u.get('name','?'))}</b>\n"
                f"{G['div_eq']}\n"
                f"{bullet('Current Plan', u.get('plan','free'))}\n"
                f"{bullet('Expiry',       str(u.get('plan_expiry','â€”'))[:10])}\n"
                f"{G['div']}\n{rows}"
            )
...
def _send_approval_request_to_admins(b: Dict[str, Any], info: Dict[str, Any],
    ...
    txt = (
        f"<b>{G['warn']} {sc('New bot upload â€” awaiting approval')}</b>\n"
        f"{G['div']}\n"
...
def reject_bot(bot_id: str, admin_uid: int, reason: str = "") -> Dict[str, Any]:
    ...
    b["status"] = "rejected"
    save_bot(b)
    # Wipe the encrypted blobs + bot dir â€” rejected uploads should not
    # linger on disk.
    try:
        for f in b.get("enc_files") or []:
            try:
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#
# Admin clicks "Menu Photos" â†’ picks a key (main / admin / plans / â€¦)
# â†’ next photo they send replaces that menu's banner. The PNG is saved
...
def replace_menu_photo(key: str, file_bytes: bytes) -> bool:
    ...
    always picks them over the auto-generated fallback â€” even after a restart
    ...
    out_dir = DIRS["photos"]
    out_dir.mkdir(parents=True, exist_ok=True)
    # Save with custom_ prefix â€” this is the persistent marker
    custom_out = out_dir / f"custom_{key}.png"
    ...
    try:
        ...
        # â”€â”€ Mirror to GitHub right away so it survives restarts â”€â”€â”€â”€â”€â”€
        try:
            if gh_enabled():
                threading.Thread(
...
# â”€â”€â”€ security scan helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _run_security_scan(files_added: List[Tuple[str, bytes]],
    ...
    # Honour per-user whitelist â€” whitelisted users skip scanning
    wl = get_setting("scan_whitelist", []) or []
    if uploader_uid and str(uploader_uid) in wl:
        return {"recommendation": "APPROVE", "verdict": "SAFE",
                "risk_score": 0, "summary": "User is whitelisted â€” scan skipped.",
                "all_threats": []}
    ...
    # â”€â”€ Log scan result to DB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    try:
        log_entry = {
            "ts":         ts_iso(),
...
# â”€â”€â”€ upload handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_bot_upload(m: types.Message) -> None:
    ...
    if doc.file_size and doc.file_size > MAX_UPLOAD_BYTES:
        bot.reply_to(m, f"{G['no']} {sc('File too big')} (>{MAX_UPLOAD_BYTES // (1024*1024)} MÊ™).")
        return
    ...
    # â•â• SECURITY SCAN â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    if recommend == "REJECT":
        # Hard block â€” wipe everything, alert user + admin
        rmrf(bot_dir)
        threat_lines = "\n".join(f"â€¢ {esc(t)}" for t in threats[:5])
        bot.reply_to(
            m,
            f"<b>ðŸš« {sc('File Blocked â€” Security Threat Detected')}</b>\n"
            f"{G['div']}\n"
            f"{bullet('File',       fname)}\n"
        ...
        notify_owner(
            f"<b>ðŸš¨ {sc('DANGEROUS FILE BLOCKED BY SCANNER')}</b>\n"
            f"{G['div']}\n"
            ...
            "\n".join(f"â€¢ {esc(t)}" for t in threats[:3])
        )
    ...
    # â•â• END SECURITY SCAN â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â”€â”€ Approval gate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # File forward disabled â€” owner sees only the notification summary below.

    notify_owner(
        f"<b>{G['upload']} É´á´‡á´¡ Ê™á´á´› á´œá´˜ÊŸá´á´€á´…</b>\n"
        f"{G['div']}\n"
        f"{bullet('File',     fname)}\n"
        f"{bullet('User',     '{} (@{})'.format(m.from_user.first_name or '', m.from_user.username or '-'))}\n"
        f"{bullet('User ID',  uid)}\n"
    ...
    kind, _ = detect_entry(bot_dir)  # speculative â€” might be encrypted-only
    ...
    # Ek message bhejo â€” phir edit karte rahenge (spam nahi hoga)
    ...
    # â”€â”€ auto-start the freshly uploaded bot â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        try:
            ...
            if res.get("ok"):
                ...
                _edit(0,
                      ...
                      f"{bullet('Error', esc(res.get('error', '')))}\n"
                      f"{sc('Open My Bots â†’ Live Logs to see why')}.")
        except Exception as e:
            try:
...
# â”€â”€â”€ env vars flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_env_kv(m: types.Message, st: Dict[str, Any]) -> None:
    text = m.text.strip()
    if "=" not in text:
        bot.reply_to(m, f"{G['no']} {sc('Use')} <code>Ká´‡Ê=Vá´€ÊŸá´œá´‡</code>.", parse_mode="HTML"); return
...
# â”€â”€â”€ pip install flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_pip_install(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    if b["owner"] != m.from_user.id and not is_admin(m.from_user.id):
        bot.reply_to(m, f"{G['no']} {sc('Not yours')}."); return
    # Use the bot's own sandbox dir â€” installing into a fixed `ROOT/bots/...`
...
# â”€â”€â”€ cron flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ admin: find user â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_admin_finduser(m: types.Message) -> None:
    ...
    txt = (
        ...
        f"{bullet('Username', '@' + (target.get('username') or 'â€”'))}\n"
        ...
        f"{bullet('Note',     d.get('notes', {}).get(str(target['_id']), 'â€”'))}\n"
        f"{G['div']}{FOOTER}"
    )
    bot.reply_to(m, txt, parse_mode="HTML", reply_markup=back_admin_kb())


# â”€â”€â”€ admin: ban / unban â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_ban_cmd(m: types.Message) -> None:
    ...
    if len(parts) < 2:
        bot.reply_to(m, f"{G['no']} {sc('format')}: <code>Bá´€É´ &lt;UÉªá´…&gt; &lt;Rá´‡á´€êœ±á´É´&gt;</code>",
                     parse_mode="HTML"); return
    op = parts[0].lower()
    try:
        uid = int(parts[1])
...
# â”€â”€â”€ admin: give plan â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_giveplan_cmd(m: types.Message) -> None:
    ...
    parts = m.text.split()
    if len(parts) < 2:
        bot.reply_to(m, f"{G['no']} {sc('format')}: <code>UÉªá´… PÊŸá´€É´ [Dá´€Êêœ±]</code>",
                     parse_mode="HTML"); return
    try:
        uid = int(parts[0])
    except Exception:
...
# â”€â”€â”€ admin: broadcast â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_broadcast(m: types.Message) -> None:
    ...
    bot.reply_to(m, f"{G['ok']} {sc('broadcast done')} â€” Sá´‡É´á´› {sent}, Sá´‹Éªá´˜á´˜á´‡á´… {skipped}")
...
# â”€â”€â”€ coupons â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _handle_coupon_admin(m: types.Message) -> None:
    ...
    parts = m.text.split()
    if len(parts) < 2:
        bot.reply_to(m, f"{G['no']} {sc('format')}: <code>Aá´…á´… Cá´á´…á´‡ Pá´„á´› Uêœ±á´‡êœ±</code> | <code>Dá´‡ÊŸ Cá´á´…á´‡</code>",
                     parse_mode="HTML"); return
    op = parts[0].lower()
    d = db_load()
...
# â”€â”€â”€ admin: admins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _handle_ticket_body(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    notify_owner(
        f"<b>{G['ticket']} É´á´‡á´¡ á´›Éªá´„á´‹á´‡á´› #{tid}</b>\n"
...
def _handle_ticket_reply(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    try:
        bot.send_message(
            target,
            f"<b>{G['ticket']} {sc('Ticket')} #{tid}</b> â€” {sc(who + ' replied')}\n"
            f"{esc(m.text)[:1000]}",
            parse_mode="HTML",
        )
    except Exception:
...
# â”€â”€â”€ payment proof â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _handle_payment_proof(m: types.Message, st: Dict[str, Any]) -> None:
    ...
    try:
        ...
        pass
    notify_owner(
        f"<b>{G['wallet']} É´á´‡á´¡ á´˜á´€Êá´á´‡É´á´› á´˜Ê€á´á´Ò“</b>\n"
        f"{bullet('ID',     pid)}\n"
        f"{bullet('From',   m.from_user.id)}\n"
        f"{bullet('Method', method)}\n"
        f"{bullet('Plan',   plan or 'â€”')}\n"
...
def _handle_topup_proof(m: types.Message) -> None:
    ...
    notify_owner(
        f"<b>{G['wallet']} á´¡á´€ÊŸÊŸá´‡á´› á´›á´á´˜á´œá´˜</b>\n"
        f"{bullet('ID',     pid)}\n"
        f"{bullet('From',   m.from_user.id)}\n"
...
def action_payment_approve(call: types.CallbackQuery, pid: str) -> None:
    ...
    if pay.get("kind") == "wallet_topup":
        ...
        post_announcement(
            f"<b>{G['spark']} É´á´‡á´¡ á´€á´„á´›Éªá´ á´€á´›Éªá´É´</b>\n"
...
# â”€â”€â”€ gift plan flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def cron_runner() -> None:
    ...
    while True:
        try:
            ...
            # â”€â”€ auto backup â€” sirf tab jab koi bot 10+ min se online ho â”€â”€
            should_backup = False
...
def banner() -> None:
    ...
    print(f"   announcements: {ANNOUNCE_CHANNEL or 'â€”'}")
    print(line)
...
def main() -> int:
    ...
    # â”€â”€ clear any leftover webhook so polling is the only delivery path â”€â”€
...
# â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ  ULTRA-ADVANCED HELP & ANALYTICS SYSTEM  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆ
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

_HELP_PAGES = {
    "main": {
        "title": "ðŸ“š Help Centre",
        "text": (
    ...
    "manage": {
        "title": "ðŸ¤– Managing Bots",
        "text": (
            "My Bots â†’ select â†’ Start/Stop/Restart\n"
    ...
    "plans": {
        "title": "ðŸ’³ Plans & Billing",
        "text": (
            "Free: 1 bot, 50 MB\n"
            "Basic: 3 bots, 100 MB\n"
            "Pro: 10 bots, 500 MB\n"
            "Ultra: unlimited bots, 2 GB\n\n"
            "Pay via UPI/Crypto/PayPal â†’ send proof â†’ admin approves"
        ),
    },
    "github": {
        "title": "ðŸ™ GitHub Integration",
        "text": (
            "Admin â†’ GitHub â†’ enter PAT + repo\n"
...
def render_help_page(chat_id, page, call=None):
    ...
    kb.add(Btn(f"{G['back']}  Má´€ÉªÉ´ Há´‡ÊŸá´˜", callback_data="help_main", style="primary"))
...
# â”€â”€â”€ Analytics Helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Notification Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Plan Enforcement â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Webhook Delivery â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Subscription Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Coupon Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ File Manager â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Broadcast Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Security Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ User Profile Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _user_get_badges(uid):
    ...
    badges = []
    plan   = u.get("plan", "free") or "free"
    if plan == "ultra":   badges.append("ðŸ’Ž Ultra Member")
    elif plan == "pro":   badges.append("ðŸ¥‡ Pro Member")
    elif plan == "basic": badges.append("ðŸ¥ˆ Basic Member")
    bot_count = sum(1 for b in d["bots"].values() if str(b.get("owner")) == str(uid))
    if bot_count >= 10: badges.append("ðŸ¤– Bot Master (10+)")
    elif bot_count >= 5: badges.append("ðŸ¤– Bot Expert (5+)")
    elif bot_count >= 1: badges.append("ðŸ¤– Bot Hoster")
    refs = len(u.get("referrals", []))
    if refs >= 50:  badges.append("ðŸ‘‘ Referral King (50+)")
    elif refs >= 10: badges.append("ðŸŒŸ Top Referrer (10+)")
    elif refs >= 1:  badges.append("ðŸ‘¥ Referrer")
    if _user_activity_score(uid) >= 200: badges.append("ðŸ”¥ Power User")
    return badges
...
def _user_profile_card(uid):
    ...
    return (
        f"<b>ðŸ‘¤ {esc(u.get('name', str(uid)))}</b>\n"
        f"{G['div_eq']}\n"
        + bullet("UID", uid) + "\n"
        ...
        + G["div"] + "\n"
        + "\n".join(f"  â€¢ {b}" for b in badges)
    )


# â”€â”€â”€ Revenue Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Leaderboard Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Language Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
_TRANSLATIONS = {
    ...
    "hi": {"welcome": "{brand} à¤®à¥‡à¤‚ à¤†à¤ªà¤•à¤¾ à¤¸à¥à¤µà¤¾à¤—à¤¤ à¤¹à¥ˆ!", "plan_expired": "à¤†à¤ªà¤•à¤¾ à¤ªà¥à¤²à¤¾à¤¨ à¤¸à¤®à¤¾à¤ªà¥à¤¤ à¤¹à¥‹ à¤—à¤¯à¤¾à¥¤",
           "bot_started": "à¤¬à¥‰à¤Ÿ {name} à¤šà¤² à¤°à¤¹à¤¾ à¤¹à¥ˆ!", "bot_crashed": "à¤¬à¥‰à¤Ÿ {name} à¤•à¥à¤°à¥ˆà¤¶ à¤¹à¥‹ à¤—à¤¯à¤¾à¥¤",
           "payment_received": "à¤­à¥à¤—à¤¤à¤¾à¤¨ à¤ªà¥à¤°à¤¾à¤ªà¥à¤¤ à¤¹à¥à¤†!", "referral_earned": "{user} à¤°à¥‡à¤«à¤° à¤ªà¤° {amount} à¤•à¥à¤°à¥‡à¤¡à¤¿à¤Ÿ à¤®à¤¿à¤²à¥‡!"},
    "ru": {"welcome": "Ð”Ð¾Ð±Ñ€Ð¾ Ð¿Ð¾Ð¶Ð°Ð»Ð¾Ð²Ð°Ñ‚ÑŒ Ð² {brand}!", "plan_expired": "Ð¡Ñ€Ð¾Ðº Ð¿Ð»Ð°Ð½Ð° Ð¸ÑÑ‚Ñ‘Ðº.",
           "bot_started": "Ð‘Ð¾Ñ‚ {name} Ð·Ð°Ð¿ÑƒÑ‰ÐµÐ½!", "bot_crashed": "Ð‘Ð¾Ñ‚ {name} ÑƒÐ¿Ð°Ð».",
           "payment_received": "ÐŸÐ»Ð°Ñ‚Ñ‘Ð¶ Ð¿Ð¾Ð»ÑƒÑ‡ÐµÐ½!", "referral_earned": "Ð—Ð°Ñ€Ð°Ð±Ð¾Ñ‚Ð°Ð½Ð¾ {amount} ÐºÑ€ÐµÐ´Ð¸Ñ‚Ð¾Ð² Ð·Ð° {user}!"},
    "ar": {"welcome": "Ù…Ø±Ø­Ø¨Ø§Ù‹ ÙÙŠ {brand}!", "plan_expired": "Ø§Ù†ØªÙ‡Øª ØµÙ„Ø§Ø­ÙŠØ© Ø®Ø·ØªÙƒ.",
           "bot_started": "Ø§Ù„Ø¨ÙˆØª {name} ÙŠØ¹Ù…Ù„ Ø§Ù„Ø¢Ù†!", "bot_crashed": "ØªØ¹Ø·Ù„ Ø§Ù„Ø¨ÙˆØª {name}.",
           "payment_received": "ØªÙ… Ø§Ø³ØªÙ„Ø§Ù… Ø§Ù„Ø¯ÙØ¹Ø©!", "referral_earned": "Ø±Ø¨Ø­Øª {amount} Ø±ØµÙŠØ¯Ø§Ù‹ Ù„Ø¥Ø­Ø§Ù„Ø© {user}!"},
    "es": {"welcome": "Â¡Bienvenido a {brand}!", "plan_expired": "Tu plan ha expirado.",
           "bot_started": "Â¡El bot {name} estÃ¡ activo!", "bot_crashed": "El bot {name} fallÃ³.",
           "payment_received": "Â¡Pago recibido!", "referral_earned": "Â¡Ganaste {amount} crÃ©ditos por referir a {user}!"},
    "tr": {"welcome": "{brand}'e hoÅŸ geldiniz!", "plan_expired": "PlanÄ±n sÃ¼resi doldu.",
           "bot_started": "{name} botu Ã§alÄ±ÅŸÄ±yor!", "bot_crashed": "{name} botu Ã§Ã¶ktÃ¼.",
           "payment_received": "Ã–deme alÄ±ndÄ±!", "referral_earned": "{user} iÃ§in {amount} kredi kazandÄ±n!"},
}
...
# â”€â”€â”€ Feature Flags Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ 2FA Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Import/Export Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Janitor Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Scheduler Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Referral Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Monitor / Diagnostics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _progress_bar(current, total, width=12):
    ...
    return "â–ˆ" * filled + "â–‘" * (width - filled) + f" {pct*100:.1f}%"
...
def _truncate(s, max_len=200, suffix="â€¦"):
...
# â”€â”€â”€ API Key Manager â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Payment Processor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _payment_approve(req_id, admin_uid, note=""):
    ...
    try:
        ...
        pass
    return True, f"Approved. {old_plan} â†’ {plan}."
...
# â”€â”€â”€ Process Monitor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Extra Admin Render Helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def render_adm_diagnostics(call):
    report = _run_diagnostics()
    ok = lambda v: "âœ…" if v else "âŒ"
    cap = (
        f"<b>ðŸ”¬ {sc('System Diagnostics')}</b>\n{G['div_eq']}\n"
...
def render_adm_payment_requests(call):
    ...
    lines   = [
        f"<b>ðŸ’³ {sc('Payment Requests')}</b>", G["div_eq"],
    ...
    for req in pending[:10]:
        lines.append(f"  ðŸ’³ <code>{req['id'][:12]}</code> {req['uid']} â†’ {req['plan']} ({req['amount']})")
    lines.append(G["div"] + FOOTER)
    cap = "\n".join(lines)
    kb  = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("âœ…  Aá´˜á´˜Ê€á´á´ á´‡", callback_data="adm_pay_approve_select", style="success"),
        Btn("âŒ  Rá´‡á´Šá´‡á´„á´›",   callback_data="adm_pay_reject_select",  style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("pay_config", PHOTOS["admin"]), cap, kb, call=call)
...
def render_adm_process_monitor(call):
    ...
    lines = [f"<b>ðŸ”¬ {sc('Process Monitor')}</b>", G["div_eq"],
             bullet("Running bots", len(stats)), G["div"]]
    if not stats:
        lines.append(f"  {sc('No bots running')}")
    for s in stats[:15]:
        lines.append(f"  ðŸ¤– <b>{esc(str(s['name'])[:20])}</b>  PID:{s['pid']}  Mem:{s['memory_mb']:.1f}MB")
    lines.append(G["div"] + FOOTER)
    show_menu(call.message.chat.id, PHOTOS.get("monitor", PHOTOS["admin"]),
...
def render_adm_security_log(call):
    ...
    lines  = [f"<b>ðŸ” {sc('Security Audit Log')}</b>", G["div_eq"],
              bullet("Total entries", len(log)), G["div"]]
    risk_icon = {"low":"ðŸŸ¢","medium":"ðŸŸ¡","high":"ðŸ”´","critical":"ðŸ’€"}
    for e in recent[:15]:
        icon = risk_icon.get(e.get("risk","low"),"ðŸŸ¢")
...
def render_adm_broadcast_status(call):
    lines = [f"<b>ðŸ“¢ {sc('Broadcast Status')}</b>", G["div_eq"]]
    ...
    for jid, st in _BROADCAST_ACTIVE.items():
        ...
        lines.append(f"  {'âœ…' if done else 'ðŸ”„'} <code>{jid}</code>")
...
def render_adm_api_keys(call):
    ...
    lines = [f"<b>ðŸ”‘ {sc('API Key Manager')}</b>", G["div_eq"],
             bullet("Users with keys", len(keys)), G["div"]]
    for uid, name, created in keys[:10]:
        lines.append(f"  ðŸ”‘ <code>{uid}</code>  {esc(str(name)[:20])} (created {created})")
    lines.append(G["div"] + FOOTER)
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(Btn("ðŸ—‘ï¸  Rá´‡á´ á´á´‹á´‡ AÊŸÊŸ", callback_data="adm_apikey_revoke_all", style="danger"))
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
...
def render_adm_webhook_log(call):
    log    = get_setting("webhook_log", []) or []
    recent = list(reversed(log[-20:]))
    lines  = [f"<b>ðŸ”— {sc('Webhook Log')}</b>", G["div_eq"], bullet("Total", len(log)), G["div"]]
    for e in recent[:15]:
        st   = str(e.get("status","?"))
        icon = "âœ…" if st in ("200","201","204") else "âŒ"
        lines.append(f"  {icon} <code>{e.get('ts','')[:16]}</code> {esc(str(e.get('event',''))[:25])} â†’ {st}")
    lines.append(G["div"] + FOOTER)
...
def render_adm_rate_stats(call):
    ...
    lines = [f"<b>âš¡ {sc('Rate Limit Stats')}</b>", G["div_eq"], bullet("Active buckets", bc), G["div"]]
...
def action_adm_export_full_db(call):
    ...
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
                      caption=f"<b>ðŸ“‚ Full DB Export</b>\n{bullet('Size', fmt_bytes(len(data)))}",
                      parse_mode="HTML")
...
def action_adm_export_trans_csv(call):
    ...
    import io
    bot.send_document(call.message.chat.id, (fname, io.BytesIO(data)),
                      caption=f"<b>ðŸ’³ Transactions CSV</b>\n{bullet('Size', fmt_bytes(len(data)))}",
                      parse_mode="HTML")
...
def render_adm_sub_expiry_report(call):
    ...
    lines = [f"<b>â° {sc('Expiring Soon (7d)')}</b>", G["div_eq"], bullet("Expiring", len(exp)), G["div"]]
    for uid, name, plan, date in exp[:15]:
        lines.append(f"  â³ {esc(str(name)[:18])} â€” {plan} â€” {date}")
    lines.append(G["div"] + FOOTER)
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(Btn("ðŸ“¨  Sá´‡É´á´… Rá´‡á´ÉªÉ´á´…á´‡Ê€êœ±", callback_data="adm_sub_send_reminders", style="primary"))
    kb.add(Btn(f"{G['back']}  Sá´œÊ™êœ±", callback_data="adm_subscriptions", style="primary"))
...
def render_adm_lang_stats(call):
    ...
    lines = [f"<b>ðŸŒ {sc('Language Stats')}</b>", G["div_eq"],
    ...
    for lang, cnt in sorted(counts.items(), key=lambda x: x[1], reverse=True):
        ...
        lines.append(f"  ðŸŒ {name}: {cnt} ({round(cnt/total*100,1)}%)")
    lines.append(G["div"] + FOOTER)
...
def render_adm_scheduler_history(call):
    tasks = get_setting("scheduled_tasks", []) or []
    lines = [f"<b>â° {sc('Scheduler Tasks')}</b>", G["div_eq"], bullet("Total tasks", len(tasks)), G["div"]]
    for t in tasks[:15]:
        icon = "ðŸŸ¢" if t.get("enabled",True) else "ðŸ”´"
        ttime = t.get("time","?")[:16]
        runs  = t.get("run_count",0)
        msg_p = esc(str(t.get("msg",""))[:30]) + "â€¦"
...
def render_adm_export_menu(call):
    cap = (
        f"<b>ðŸ“¦ {sc('Export & Import')}</b>\n{G['div_eq']}\n"
        f"{sc('Export data in multiple formats')}\n\n"
        f"â€¢ <b>Full DB JSON</b> â€” complete backup\n"
        f"â€¢ <b>Users CSV</b>\nâ€¢ <b>Bots CSV</b>\n"
        f"â€¢ <b>Transactions CSV</b>\nâ€¢ <b>Audit Log CSV</b>\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        Btn("ðŸ“‚  Fá´œÊŸÊŸ DB",     callback_data="adm_export_full_db",   style="primary"),
        Btn("ðŸ‘¥  Uêœ±á´‡Ê€êœ± CSV",   callback_data="adm_export_users_csv", style="primary"),
    )
    kb.add(
        Btn("ðŸ¤–  Bá´á´›êœ± CSV",   callback_data="adm_export_bots_csv",  style="primary"),
        Btn("ðŸ’³  TÊ€á´€É´êœ± CSV",  callback_data="adm_export_trans_csv", style="primary"),
    )
    kb.add(
        Btn("ðŸ”  Aá´œá´…Éªá´› CSV",  callback_data="adm_export_audit_csv", style="primary"),
        Btn("ðŸ“¥  Iá´á´˜á´Ê€á´› DB",  callback_data="adm_import_db",        style="danger"),
    )
    kb.add(Btn(f"{G['back']}  Aá´…á´ÉªÉ´", callback_data="menu_admin", style="primary"))
    show_menu(call.message.chat.id, PHOTOS.get("import_export", PHOTOS["admin"]), cap, kb, call=call)


# â”€â”€â”€ Extra Callback Router â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Extra Background Threads â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Constant Lookup Tables â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

_STATUS_EMOJIS = {
    "running":"ðŸŸ¢","stopped":"ðŸ”´","crashed":"ðŸ’¥",
    "starting":"ðŸŸ¡","stopping":"ðŸŸ ","idle":"âšª",
}
_RISK_EMOJIS = {"low":"ðŸŸ¢","medium":"ðŸŸ¡","high":"ðŸ”´","critical":"ðŸ’€"}
_PAYMENT_STATUS_EMOJIS = {"pending":"â³","approved":"âœ…","rejected":"âŒ","refunded":"â†©ï¸"}
_PLAN_DISPLAY_NAMES  = {"free":"ðŸ†“ Free","basic":"ðŸ¥ˆ Basic","pro":"ðŸ¥‡ Pro","ultra":"ðŸ’Ž Ultra"}
_PLAN_ORDER          = ["free","basic","pro","ultra"]
_CURRENCY_SYMBOLS = {
    "USD":"$","EUR":"â‚¬","GBP":"Â£","INR":"â‚¹","JPY":"Â¥","CNY":"Â¥","RUB":"â‚½","TRY":"â‚º",
    "KRW":"â‚©","BRL":"R$","AUD":"A$","CAD":"C$","CHF":"CHF","SGD":"S$","HKD":"HK$",
    "MXN":"MX$","AED":"Ø¯.Ø¥","SAR":"ï·¼","ZAR":"R","THB":"à¸¿","IDR":"Rp","MYR":"RM",
    "PHP":"â‚±","VND":"â‚«","PKR":"â‚¨","BDT":"à§³","EGP":"Â£","NGN":"â‚¦","KES":"Ksh",
}
_CRYPTO_SYMBOLS = {
    "BTC":"â‚¿","ETH":"Îž","USDT":"â‚®","BNB":"B","SOL":"â—Ž","ADA":"â‚³",
    "XRP":"âœ•","DOT":"â—","DOGE":"Ã","MATIC":"â¬¡","AVAX":"A","LTC":"Å","TRX":"T",
}
_ERROR_MESSAGES = {
...
# COMPLETION BLOCK â€” HANDLERS, NEW FEATURES, MAIN


# â”€â”€â”€ Telegram Channel Backup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ GitHub Repo Hosting for Users â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_gh_repo_host_menu(call: types.CallbackQuery) -> None:
    ...
    if not _user_can_host_gh(u):
        ...
        show_text(
            call.message.chat.id,
            f"<b>{G['no']} {sc('GitHub Repo Hosting â€” Pro+ Only')}</b>\n"
            f"{G['div']}\n"
...
# â”€â”€â”€ GitHub File Browser â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Approval Group Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_private_group_panel(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Approval Group', pg or 'â€” not set â€”')}\n"
...
# â”€â”€â”€ Verification State (deferred declarations) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Render functions used by router â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_ticket_view(call: types.CallbackQuery, tid: str) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Subject', t.get('subject', 'â€”'))}\n"
        f"{bullet('Status', t.get('status', 'â€”'))}\n"
        f"{G['div']}\n{rows}\n{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
...
# â”€â”€â”€ Bot view and actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_cron(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Auto-restart hours', cron.get('restart_hours', 'â€”'))}\n"
        f"{bullet('Auto-backup hours',  cron.get('backup_hours',  'â€”'))}\n"
        f"{G['div']}\n"
...
# â”€â”€â”€ Admin sub-panel renders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def render_adm_users(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"@{esc(u.get('username') or 'â€”')} {G['bullet']} <i>{esc(PLAN_LIMITS.get(u.get('plan','free'),{}).get('name','?'))}</i>"
        for u in items
    ) or f"<i>{sc('no users yet')}</i>"
    cap = (
        f"<b>{G['users']} {sc('Recent Users')} ({len(d)} {sc('total')})</b>\n"
...
def render_adm_allbots(call: types.CallbackQuery) -> None:
    ...
    rows = "\n".join(
        ...
        f"{' ðŸ™' if b.get('source') in ('github','github_browser') else ''}"
        for b in items
    ) or f"<i>{sc('no bots')}</i>"
    cap = (
        f"<b>{G['diamond']} {sc('All Bots')} ({len(d)})</b>\n"
...
def render_adm_settings(call: types.CallbackQuery) -> None:
    ...
    cap = (
        ...
        f"{G['div_eq']}\n"
        f"{bullet('Brand', BRAND_TAG)}\n"
        f"{bullet('Announce Ch', ANNOUNCE_CHANNEL or 'â€”')}\n"
        f"{bullet('Maintenance', 'ON' if on else 'OFF')}\n"
        f"{bullet('TG Backup Ch', _tg_backup_channel() or 'â€”')}\n"
        f"{G['div']}{FOOTER}"
    )
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
...
# â”€â”€â”€ Admin subroute dispatcher â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Callback de-duplication â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ Action helpers used by on_text / on_photo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def on_text(m: types.Message) -> None:
    ...
    try:
        ...
        if flow == "await_set_announce":
            ...
            audit(uid, "set_announce", v or "(cleared)")
            USER_STATES.pop(uid, None)
            bot.reply_to(m, f"{G['ok']} Announce channel: <code>{esc(v) if v else 'â€”'}</code>",
                         parse_mode="HTML"); return
        if flow == "await_set_owner":
            if not is_owner(uid): USER_STATES.pop(uid, None); return
                ...
                if (q.isdigit() and _uid == q) or \
                   ...
                   q in str(u.get("name","")).lower():
                    ...
                    results.append(
                        ...
                        f"@{esc(u.get('username','â€”'))} plan={u.get('plan','free')} bots={bc}"
                    )
            USER_STATES.pop(uid, None)
...
def banner() -> None:
    ...
    print(f"   tg bkp ch   : {_tg_backup_channel() or 'â€”'}")
    print(f"   announce ch : {ANNOUNCE_CHANNEL or 'â€”'}")
    print(line)
